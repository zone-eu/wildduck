...
const SETTING_KEYS = [
    {
        ...
        schema: { type: 'number', wdType: 'number' }
    },
    ...
    {
        key: 'const:imap:use_wd_worker_channels',
        name: 'Use worker wd_events channels',
        description: 'Publish IMAP notifications to worker-specific wd_events channels instead of the legacy shared wd_events channel',
        type: 'boolean',
        confValue: false,
        schema: { type: 'boolean', wdType: 'booleanStrict' }
    },
...

    {
        ...
        // Joi allowed the empty string next to the pattern (.allow('').trim().pattern(...))
        schema: { type: 'string', wdTrim: true, anyOf: [{ const: '' }, { pattern: '^\\d+\\s*[a-z]*(\\s*,\\s*\\d+\\s*[a-z]*)*$' }] }
    },

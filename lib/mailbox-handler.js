...
class MailboxHandler {
    constructor(options) {
        ...
        // JMAP changelog helper
        try {
            const JmapChanges = require('./jmap-changes');
            this.jmapChanges = new JmapChanges(this.redis);
        } catch (E) {
            this.jmapChanges = null;
        }
    }
                        ...
                        this.database.collection('mailboxes').findOneAndUpdate(
                            ...
                            (err, item) => {
                                ...
                                this.notifier.addEntries(
                                    ...
                                    async () => {
                                        // append updated changelog entries for messages in this mailbox (best-effort)
                                        // Use bulk operation instead of sequential writes for performance
                                        try {
                                            if (this.jmapChanges) {
                                                const messageIds = [];
                                                const cursor = this.database.collection('messages').find({ mailbox }, { projection: { _id: 1 } });
                                                
                                                while (await cursor.hasNext()) {
                                                    const doc = await cursor.next();
                                                    messageIds.push(doc._id.toString());
                                                }
                                                
                                                // Batch append changes using single bulk operation
                                                if (messageIds.length > 0) {
                                                    try {
                                                        await this.jmapChanges.appendChangesBulk(mailboxData.user, messageIds.map(id => ({ type: 'updated', id })));
                                                    } catch (E) {
                                                        // Fallback to individual appends only on pipeline error
                                                        this.loggelf({
                                                            user: mailboxData.user,
                                                            mailbox,
                                                            action: 'jmap_changelog_bulk_fallback',
                                            ...
                                            // ignore changelog errors but log for debugging
                                            this.loggelf({
                                                user: mailboxData.user,
                                                mailbox,
                                                action: 'jmap_changelog_sync',
                                                error: E.message
                                            });
                                        }
                                    ...
                                    () => {
                                        this.database.collection('messages').updateMany(
                                            ...
                                            async err => {
                                                ...
                                                // Append destroyed changelog entries for messages in this mailbox (best-effort)
                                                // Use bulk operation instead of sequential writes for performance
                                                try {
                                                    if (this.jmapChanges) {
                                                        const messageIds = [];
                                                        ...
                                                        if (messageIds.length > 0) {
                                                            try {
                                                                await this.jmapChanges.appendChangesBulk(mailboxData.user, messageIds.map(id => ({ type: 'destroyed', id })));
                                                            } catch (E) {
                                                                // Log bulk operation failures
                                                                this.loggelf({
                                                                    user: mailboxData.user,
                                                                    mailbox,
                                                                    action: 'jmap_changelog_bulk_destroy',
                                                                    count: messageIds.length,
                                                                    error: E.message
                                                                });
                                                            }

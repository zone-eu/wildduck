...
function createToolRunner(observe, checkLimit) {
    return async (name, action) => {
        ...
        try {
            ...
            let text = JSON.stringify(output, false, 2);
    ...
    tool(
        ...
        ACTION.READ,
        {
            ...
            inputSchema: z.strictObject({
                ...
                searchable: z.boolean().optional().describe('If true, skips Junk and Trash'),
                order: z.enum(['asc', 'desc']).default('desc'),
        ...
        async args =>
            run('search_messages', async () => {
                let mailbox = args.mailbox ? (await reader.resolveMailbox(args.mailbox)).id : undefined;
    ...
    tool(
        ...
        async ({ mailbox, uid, body_format: bodyFormat }) =>
            run('get_message', async () => {
                let mailboxData = await reader.resolveMailbox(mailbox);
                let message = await reader.getMessage({ mailbox: mailboxData.id, message: uid });

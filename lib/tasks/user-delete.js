...
const run = async (task, data) => {
    ...
    try {
        // Alongside asps: both are credentials for one account, so they go on the same
        // schedule. Authentication already fails the moment the user row moves to
        // `deletedusers`, so this is the housekeeping half of a revocation that has happened.
        let delRes = await db.users.collection('mcptokens').deleteMany({ user: data.user });
        result.mcptokens = { deleted: delRes.deletedCount };
    } catch (err) {
        log.error('Tasks', 'task=user-delete id=%s user=%s message=%s error=%s', task._id, data.user, 'Failed to delete MCP tokens', err.message);
        err.code = 'InternalDatabaseError';
        result.mcptokens = { error: err.message };
        throw err;
    }
    ...
    try {
        let delRes = await db.database.collection('pushsubscriptions').deleteMany({ user: data.user });
        result.pushsubscriptions = { deleted: delRes.deletedCount };
    } catch (err) {
        log.error('Tasks', 'task=user-delete id=%s user=%s message=%s error=%s', task._id, data.user, 'Failed to delete push subscriptions', err.message);
        err.code = 'InternalDatabaseError';
        result.pushsubscriptions = { error: err.message };
        throw err;
    }

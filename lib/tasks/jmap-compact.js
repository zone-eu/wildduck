...
module.exports = async function (task, data, env, callback) {
    ...
    // Ensure TTL index exists on jmap_changes collection (30 days retention)
    // This prevents unbounded growth of the MongoDB changelog archive
    try {
        await db.database.collection('jmap_changes').createIndex(
            { ts: 1 },
            { 
                expireAfterSeconds: 30 * 24 * 60 * 60, // 30 days
                name: 'jmap_changes_ttl',
                background: true
            }
        );
    } catch (E) {

...
let run = async (task, data, options) => {
    ...
    if (data.q) {
        filter = await getMongoDBQuery(db, user, data.q, { useAndSearch: data.useAndSearch, searchable: data.searchable });
        query = data.q;
    } else {
    ...
    try {
        ...
        // the initial maximum excludes new documents inserted by our own moves.
        ...
        while (hasMore) {
            ...
            for (const { _id } of messageIds) {
                ...
                let messageAction = updateAction;
                if (messageAction === 'move' && action.moveTo.toString() === messageData.mailbox.toString()) {
                    if (!hasUpdates) {
                        continue;
                    }
                    // Messages already in the destination only need flag updates.
                    // Keep the move action for subsequent matches in other mailboxes.
                    ...
                    case 'move':
                        try {
                            await messageHandler.moveAsync({
                                ...
                                updates: hasUpdates ? updates : false,
        ...
        if (lastError) {
            // Retry failed messages through the task queue after trying the other matches.
            throw lastError;
        }
    } finally {

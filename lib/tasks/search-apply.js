...
let run = async (task, data, options) => {
    ...
    try {
        ...
        // Materialize matching message IDs before applying any action. Updating or
        // moving messages while iterating this cursor can change the indexed fields
        // used by the search and cause MongoDB to skip later matches.
        const messageIds = await db.database
            .collection('messages')
            .find(filter)
            .project({ _id: true })
            .toArray();
        ...
        for (const messageId of messageIds) {
            const messageData = await db.database.collection('messages').findOne({ _id: messageId._id });

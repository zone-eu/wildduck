...
const { getKeywordMaps, keywordPathsForMessage } = require('../keyword-handler');
...
async function copyHandler(server, messageHandler, connection, mailbox, update, session) {
    ...
    try {
        while ((messageData = await cursor.next())) {
            ...
            if (!keywordMaps && messageData.keywords?.length) {
            ...
            let entry = {
                ...
                keywords: keywordPathsForMessage(messageData, keywordMaps?.byId || new Map()),
...
module.exports = (server, messageHandler) => (connection, mailbox, update, session, callback) => {
    ...
    copyHandler(server, messageHandler, connection, mailbox, update, session)
        .then(args => {
            args = [].concat(args || []);
            endMetric(args[0] === true ? 'success' : 'error');
            callback(null, ...args);
        })
        .catch(err => {
            endMetric('error');

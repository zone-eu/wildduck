...
const { getKeywordMaps } = require('../keyword-handler');
...
module.exports = server => (mailbox, options, session, callback) => {
    db.database.collection('mailboxes').findOne(
        ...
        async (err, mailboxData) => {
            ...
            const hasKeywordQuery = nodes =>
                [].concat(nodes || []).some(term => {
                    if (term.key === 'flag') {
                        return tools.extractKeywords([term.value]).length > 0;
                    }
                    if (term.key === 'not' || term.key === 'or') {
                        return hasKeywordQuery(term.value);
                    }
                    return false;
                });
            let keywordMaps = { records: [] };
            if (hasKeywordQuery(options.query)) {
                try {
                        ...
                        case 'flag':
                            {
                                switch (term.value) {
                                    ...
                                    default:
                                        {
                                            const keyword = keywordsByLowerPath.get(term.value.toLowerCase());
                                            const shouldExist = term.exists !== ne;
                                            if (keyword && shouldExist) {
                                                parent.push({
                                                    $or: [{ keywords: keyword._id }, { flags: keyword.path }]
                                                });
                                            } else if (keyword) {
                                                parent.push({
                                                    $and: [{ keywords: { $ne: keyword._id } }, { flags: { $ne: keyword.path } }]
                                                });
                                            } else {
                                                parent.push({
                                                    flags: {

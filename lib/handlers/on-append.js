                ...
                userCache.get(session.user.id, 'imapMaxUpload', { setting: 'const:max:imap:upload' }, (err, limit) => {
                    ...
                    messageHandler.counters.ttlcounter('iup:' + session.user.id, 0, limit, false, (err, res) => {
                        ...
                        messageHandler.counters.ttlcounter('iup:' + session.user.id, raw.length, limit, false, () => {
                            ...
                            (async () => {
                                const keywordPaths = tools.extractKeywords(flags);
                                const keywordResult = await ensureKeywords(db.database, session.user.id, keywordPaths);
                                const keywords = keywordResult.keywords.map(keyword => keyword._id);
                                const keywordSet = new Set(keywordPaths.map(keyword => keyword.toLowerCase()));
                                flags = flags.filter(flag => !keywordSet.has(flag.toLowerCase()));

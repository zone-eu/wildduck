...
// shared fastify constructor options; router options and request logging
// control must not be top-level options: fastify 5 deprecates them there
// (FSTDEP022/FSTDEP023) and fastify 6 silently ignores them
const baseServerOptions = () => ({
    ...
    // servers log through Fastify's built-in pino JSON logger; request and
    // response logging is done by the hooks in this module
    logger: { level: 'warn' },
...
/**
 * HTTP status an error names, or 0 when it names none.
 *
 * `responseCode` is what handlers and the IMAP error mapping set; fastify's own
 * errors (FST_ERR_* and serialization failures) carry `statusCode` instead.
 * With clientOnly a 5xx is ignored, so a server fault raised before any handler
 * ran is reported as unhandled instead of being passed through as-is.
 */
const statusFromError = (err, { clientOnly = false } = {}) => {
    if (err.responseCode) {
    ...
    return clientOnly && err.statusCode >= 500 ? 0 : err.statusCode;
};
...
const attachPayloadStash = app => {
    app.addHook('preSerialization', async (request, reply, payload) => {
        if (payload && typeof payload === 'object') {
            ...
            if ('error' in payload && payload.success === undefined) {
...
const attachErrorHandler = (app, logTag) => {
    app.setErrorHandler((err, request, reply) => {
        ...
        if (request.wdValidated) {
            ...
            // a plain thrown Error keeps the 200 default here, because the
            // restify implementation did the same and legacy paths rely on it
        ...
        // before a handler ran, only a 4xx is a routine client error; anything
        // else is an unhandled server fault and is logged and answered as 500
        const status = statusFromError(err, { clientOnly: true });
        ...
        return reply.send({
            code: (status && err.code) || 'InternalError',
            message: err.message
        });
    });
};

...
const attachErrorHandler = (app, logTag) => {
    app.setErrorHandler((err, request, reply) => {
        ...
        // fastify's own framework errors (FST_ERR_CTP_BODY_TOO_LARGE and
        // friends) carry statusCode instead of responseCode; a 4xx one is a
        // routine client error, not an unhandled server fault
        const status = err.responseCode || (Number.isInteger(err.statusCode) && err.statusCode >= 400 && err.statusCode < 500 ? err.statusCode : 0);

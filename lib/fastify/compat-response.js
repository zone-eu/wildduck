...
class CompatResponse extends Writable {
    ...
    json(body) {
        ...
        if (body && typeof body === 'object' && body.error !== undefined && body.success === undefined) {
            // legacy inline error bodies ({error, code}, sometimes with a 200
            // status) predate the response schemas; serialize them verbatim
            // like restify did instead of letting fast-json-stringify reject
            // them over missing required keys
            this.bypassResponseSchema();
        }

...
const fileIdParam = objectIdSchema('ID of the File', { wdRequired: true });
...
module.exports = (db, server, storageHandler) => {
    server.route({
        ...
        async handler(req, reply) {
            ...
            let id = await storageHandler.add(user, values);
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let file = new ObjectId(values.file);
            try {
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            stream.once('error', err => {
                log.error('API', 'STORAGEFAIL user=%s file=%s error=%s', user, file, err.message);
            });

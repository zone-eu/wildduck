    ...
    server.post(
                    ...
                    200: {
                        ...
                        model: Joi.object({
                            ...
                            token: Joi.string().description(
                                'If access token was requested and no 2FA is required then this is the value to use as access token when making API requests on behalf of logged in user.'
                            ),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (result.value.token) {
                try {
                    if (Array.isArray(authData.require2fa) && authData.require2fa.length) {
                        authResponse.twoFactorNonce = await userHandler.generatePending2faNonce(authData.user, {
                            scope: authData.scope,
                            methods: authData.require2fa,
                            tokenRequested: true
                        });
                        ...
                        if (authData.require2fa.includes('totp')) {

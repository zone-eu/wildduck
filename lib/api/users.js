...
module.exports = (db, server, userHandler, settingsHandler) => {
    ...
    server.post(
        {
            ...
            validationObjs: {
                requestBody: {
                    ...
                    require2faEnabled: booleanSchema.default(false).description('If true then the account is flagged as requiring 2FA to be enabled'),
                    ...
                    smimeCerts: Joi.array()
                        .items(
                            Joi.string()
                                .trim()
                                .regex(/^-----BEGIN CERTIFICATE-----/, 'PEM certificate format')
                        )
                        .max(16)
                        .default([])
                        .description(
                    ...
                    smimeCipher: Joi.string()
                        .valid(...SMIMEEncryptor.CIPHERS)
                        .default(consts.SMIME_DEFAULT_CIPHER)
                        .description('S/MIME content encryption cipher.'),
                    smimeKeyTransport: Joi.string()
                        .valid(...SMIMEEncryptor.RSA_KEY_TRANSPORTS)
    ...
    server.get(
                ...
                response: {
                    200: {
                        ...
                        model: Joi.object({
                            ...
                            smimeCerts: Joi.array().items(Joi.string()).description('S/MIME recipient certificates (PEM)'),
                            smimeCipher: Joi.string()
                                .valid(...SMIMEEncryptor.CIPHERS)
                                .description('S/MIME content encryption cipher'),
                            smimeKeyTransport: Joi.string()
                                .valid(...SMIMEEncryptor.RSA_KEY_TRANSPORTS)
                                .description('S/MIME RSA key transport algorithm'),
                            smimeCertInfo: Joi.array()
                                .items(
                                    Joi.object({
                                        subject: Joi.string().required().description('Certificate subject CN'),
                                        serial: Joi.string().required().description('Certificate serial number'),
                                        fingerprint: Joi.string().required().description('SHA-256 fingerprint of the certificate')
                                    }).$_setFlag('objectName', 'SmimeCertInfoItem')
                                )
                            ...
                            require2faEnabled: booleanSchema.required().description('If true then the account is flagged as requiring 2FA to be enabled'),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let response;
            try {
                const userKey = tools.redisHashTag(db.redis, userData._id.toString());
                response = await db.redis
                    ...
                    .hget(`lim:imap:${userKey}`, userData._id.toString())
    ...
    server.put(
        {
            ...
            validationObjs: {
                requestBody: {
                    existingPassword: Joi.string().empty('').min(1).max(256).description('Validates against account password before applying any changes.'),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (req.user && req.user === result.value.user && !result.value.existingPassword) {
                res.status(400);
                return res.json({
                    error: '"existingPassword" is required',
                    code: 'InputValidationError'
                });
            }

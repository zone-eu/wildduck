...
module.exports = (db, server, userHandler, settingsHandler) => {
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let requiredTags = (values.requiredTags || '')
                .split(',')
        ...
        async handler(req, reply) {
            ...
            if (values.mailboxes) {
                ...
                for (let key of ['sent', 'junk', 'trash', 'drafts']) {
                    ...
                    // The value has to be read before the old key is deleted,
                    // otherwise every custom name is stored as undefined and
                    // silently replaced by the built-in one
                    const mailboxName = values.mailboxes[key];
                    delete values.mailboxes[key];
                    values.mailboxes[key.replace(/^./, c => '\\' + c.toUpperCase())] = mailboxName;
                }
            }
                    ...
                    200: {
                        ...
                        model: {
                            ...
                            properties: {
                                ...
                                // typeless: users created without a name store null, which
                                // must survive serialization instead of turning into ""
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            if (!values.name && 'name' in req.rawParams) {
                values.name = '';
            }

            if (!values.pubKey && 'pubKey' in req.rawParams) {
                values.pubKey = '';
            }
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let success;
            try {
                success = await userHandler.logout(values.user, values.reason || 'Logout requested from API');
            } catch (err) {
                return reply.code(err.responseCode || 500).send({
                    error: err.message,
                    code: err.code
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            // stream the export directly on the raw response; an error mid-stream is
            // reported as a trailer since the 200 header is already on the wire
            server.beginRawResponse(req, reply, 200, { 'Content-Type': 'application/octet-stream' });

            exporter.pipe(reply.raw);
            ...
            try {
                await new Promise((resolve, reject) => {
                    ...
                    runExport(values, exporter).then(resolve).catch(reject);
                });
    ...
    server.route({
        ...
        config: {
            ...
            validationObjs: {
                requestBody: {
                    validAfter: dateOrFalse('Allow using the generated password not earlier than provided time'),
                    sess: sessSchema,
                    ip: sessIPSchema
                },
                queryParams: {},
        ...
        async handler(req, reply) {
            ...
            let password;
            try {
                password = await userHandler.reset(user, values);
            } catch (err) {
                return reply.code(err.responseCode || 500).send({
                    error: err.message,
                    code: err.code
            ...
            return reply.send({
                success: true,
                password,
                validAfter: (values && values.validAfter) || new Date()
            });
        }
    });
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let deleteResponse;
            try {
                deleteResponse = await userHandler.delete(user, Object.assign({}, values));
            } catch (err) {
                return reply.code(err.responseCode || 500).send({
                    error: err.message,
                    code: err.code
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let task;
            try {
                task = await userHandler.restore(user, Object.assign({}, values));
            } catch (err) {
                return reply.code(err.responseCode || 500).send({
                    error: err.message,
                    code: err.code

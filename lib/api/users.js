    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                ...
                let userRateKey = tools.redisClusterValue(userId, db.redis);
                ...
                let [
                    ...
                    receivedResponse,
                    imapUploadResponse,
                    imapDownloadResponse,
                    pop3DownloadResponse,
                    imapConnection
                ] = await Promise.all([
                    db.redis.multi().get(`wdr:${userId}`).ttl(`wdr:${userId}`).exec(),
                    db.redis.multi().get(`wdf:${userId}`).ttl(`wdf:${userId}`).exec(),
                    db.redis.multi().get(`rl:rcpt:${userId}`).ttl(`rl:rcpt:${userId}`).exec(),
                    db.redis.multi().get(`iup:${userRateKey}`).ttl(`iup:${userRateKey}`).exec(),
                    db.redis.multi().get(`idw:${userRateKey}`).ttl(`idw:${userRateKey}`).exec(),
                    db.redis.multi().get(`pdw:${userRateKey}`).ttl(`pdw:${userRateKey}`).exec(),
                    db.redis.hget('lim:imap', userId)
                ]);

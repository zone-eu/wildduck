...
module.exports = (db, server, messageHandler, mailboxHandler, userHandler, storageHandler, notifier, settingsHandler) => {
    ...
    // Track concurrent requests per user for rate limiting
    const concurrentRequests = new Map();
    const maxConcurrentRequests = (config.jmap && config.jmap.maxConcurrentRequests) || 4;
    
    // Middleware to enforce concurrent request limits (RFC 8620 Section 3.6)
    function rateLimitMiddleware(req, res, next) {
        ...
        if (current >= maxConcurrentRequests) {
            res.status(503);
            return res.json({
                error: 'Too many concurrent requests. Please wait and try again.',
    ...
    // Helper: Authenticate with bcrypt caching to avoid repeated expensive hash operations
    // Caches successful Basic auth for 60 seconds in Redis to prevent bcrypt on every request
    async function authenticateRequest(req) {
        // Check if already authenticated via token middleware
        ...
        // Check auth cache (60 second TTL to avoid bcrypt on every request)
        const authCacheKey = `jmap:auth:${crypto.createHash('sha256').update(creds).digest('hex')}`;
        try {
            const cachedUserId = await db.redis.get(authCacheKey);
            if (cachedUserId && ObjectId.isValid(cachedUserId)) {
                req.user = cachedUserId;
                return cachedUserId;
            }
        } catch (E) {
            // ignore cache errors
        ...
        // Perform bcrypt authentication
        try {
            const [authData] = await userHandler.asyncAuthenticate(username, password, 'master', { ip: (req.params && req.params.ip) || req.connection.remoteAddress });
            if (!authData) {
                return null;
            }
            
            const userId = authData.user.toString();
            req.user = userId;
            
            // Cache successful auth for 60 seconds
            try {
                await db.redis.setex(authCacheKey, 60, userId);
            } catch (E) {
                // ignore cache errors
            }
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const apiUrl = (req.headers.host ? (req.headers['x-forwarded-proto'] || (config.api && config.api.secure ? 'https' : 'http')) + '://' + req.headers.host : '') + basePath;
            const uploadUrl = (req.headers.host ? (req.headers['x-forwarded-proto'] || (config.api && config.api.secure ? 'https' : 'http')) + '://' + req.headers.host : '') + basePath + '/upload/{accountId}';
            const eventSourceUrl = (req.headers.host ? (req.headers['x-forwarded-proto'] || (config.api && config.api.secure ? 'https' : 'http')) + '://' + req.headers.host : '') + basePath + '/eventsource?types={types}&closeafter={closeafter}&ping={ping}';
            // RFC 8620 Section 6.2: downloadUrl MUST use URI template format with variables
            const downloadUrlTemplate = (req.headers.host ? (req.headers['x-forwarded-proto'] || (config.api && config.api.secure ? 'https' : 'http')) + '://' + req.headers.host : '') + '/users/{accountId}/attachment/{blobId}/{name}?t={type}';

            const accountId = userId;
            
            // RFC 8620 Section 2: Capabilities MUST include implementation limits
            const maxUploadSize = (config.jmap && config.jmap.maxUploadMB ? Number(config.jmap.maxUploadMB) * 1024 * 1024 : 25 * 1024 * 1024);
            ...
            return res.json({
                ...
                capabilities: { 
                    'urn:ietf:params:jmap:core': {
                        ...
                        maxConcurrentUpload: config.jmap && config.jmap.maxConcurrentUpload || 4,
                        maxSizeRequest: config.jmap && config.jmap.maxSizeRequest || 10 * 1024 * 1024,
                        maxCallsInRequest: config.jmap && config.jmap.maxCallsInRequest || 32,
                        maxObjectsInGet: config.jmap && config.jmap.maxObjectsInGet || 500,
                        maxObjectsInSet: config.jmap && config.jmap.maxObjectsInSet || 500,
                        collationAlgorithms: []
                    }, 
                    'urn:ietf:params:jmap:mail': {
                        maxMailboxesPerEmail: 1,
                        maxSizeMailboxName: 255,
                        maxSizeAttachmentsPerEmail: maxUploadSize,
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const usingCapabilities = body.using || [];
            ...
            // RFC 8620 Section 3.3: Validate "using" capabilities
            // Server MUST reject methods from capabilities not listed in "using"
            const supportedCapabilities = ['urn:ietf:params:jmap:core', 'urn:ietf:params:jmap:mail'];
            const requiredCapabilities = {
                ...
                'EmailSubmission/set': ['urn:ietf:params:jmap:mail']
            };
            
            // Validate that all used capabilities are in supportedCapabilities
            for (const cap of usingCapabilities) {
                if (!supportedCapabilities.includes(cap)) {
                    res.status(400);
                    return res.json({ 
                        error: `Capability ${cap} is not supported`,
            ...
            for (let i = 0; i < methodCalls.length; i++) {
                ...
                // Validate that method's required capabilities are in "using" array (RFC 8620 Section 3.3)
                if (methodName && requiredCapabilities[methodName]) {
                    const missingCaps = requiredCapabilities[methodName].filter(cap => !usingCapabilities.includes(cap));
                    if (missingCaps.length > 0) {
                        responses.push(['error', {
                            type: 'unknownMethod',
                            description: `Method ${methodName} requires capabilities: ${missingCaps.join(', ')}`
                        }, callId]);
                        continue;
                    }
                }
                ...
                try {
                    if (methodName === 'Mailbox/get') {
                        ...
                        // Use WildDuck's cached Redis counters (avoids expensive aggregation)
                        // These counters are maintained incrementally by lib/imap-notifier.js
                        if (list.length > 0) {
                            try {
                                // Fetch all counters in parallel using Redis cached values
                                const counterPromises = list.map(async mb => {
                                    try {
                                        const total = await tools.getMailboxCounter(db, new ObjectId(mb.id));
                                        const unseen = await tools.getMailboxCounter(db, new ObjectId(mb.id), 'unseen');
                                        return { id: mb.id, total, unseen };
                                    } catch (e) {
                                        return { id: mb.id, total: 0, unseen: 0 };
                                    }
                                });
                                ...
                                for (const mb of list) {
                                    const count = counterMap.get(mb.id);
                                    if (count) {
                                        mb.totalEmails = count.total || 0;
                                        mb.unreadEmails = count.unseen || 0;
                                        mb.totalThreads = count.total || 0; // simplified
                                        mb.unreadThreads = count.unseen || 0;
                                    }
                                }
                            } catch (e) {
                                // ignore count errors
                    ...
                    if (methodName === 'Email/query') {
                        ...
                        if (args.filter && typeof args.filter === 'object') {
                            ...
                            // Support text search (with regex escaping to prevent ReDoS)
                            if (args.filter.text) {
                                const escapedText = args.filter.text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                                filter.$or = [
                                    { subject: { $regex: escapedText, $options: 'i' } },
                                    { text: { $regex: escapedText, $options: 'i' } }
                                ];
                            }
                            
                            if (args.filter.subject) {
                                const escapedSubject = args.filter.subject.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                                filter.subject = { $regex: escapedSubject, $options: 'i' };
                            }
                        }
                        ...
                        if (args.update) {
                            for (let id of Object.keys(args.update)) {
                                ...
                                try {
                                    ...
                                    // Use messageHandler.update() for proper IMAP integration (modseq, notifications, counters)
                                    if (patch.keywords !== undefined) {
                                        needsUpdate = true;
                                        const changes = {};
                                        
                                        // Map JMAP keywords to messageHandler changes
                                        // Presence = set, absence = unset
                                        if (patch.keywords['$seen']) {
                                            changes.seen = true;
                                        } else {
                                            changes.seen = false;
                                        }
                                        
                                        if (patch.keywords['$flagged']) {
                                            changes.flagged = true;
                                        } else {
                                            changes.flagged = false;
                                        }
                                        
                                        if (patch.keywords['$draft']) {
                                            changes.draft = true;
                                        } else {
                                            changes.draft = false;
                                        }
                                        
                                        // Use messageHandler.update() which properly:
                                        // - Increments modseq (CONDSTORE requirement)
                                        // - Sends IMAP notifications to IDLE clients
                                        // - Updates mailbox counters
                                    ...
                                    // Use messageHandler.move() for proper IMAP integration
                                    if (patch.mailboxIds && typeof patch.mailboxIds === 'object') {
                                        ...
                                        if (newMailboxIds.length > 0 && ObjectId.isValid(newMailboxIds[0])) {
                                            const targetMailboxId = new ObjectId(newMailboxIds[0]);
                                            
                                            // Verify mailbox exists and belongs to user
                                            const mbExists = await db.database.collection('mailboxes')
                                                .findOne({ _id: targetMailboxId, user: new ObjectId(userId) });
                                            
                                            if (mbExists && !msg.mailbox.equals(targetMailboxId)) {
                                                needsUpdate = true;
                                                // Use messageHandler.move() which properly:
                                                // - Updates uidNext in both source and target mailboxes
                                                // - Updates mailbox counters
                                                // - Sends EXPUNGE to source, EXISTS to target
                                                // - Creates journal entries
                                                // - Updates message with new mailbox/uid
                                                await new Promise((resolve, reject) => {
                                                    messageHandler.move(
                                                        {
                                                            user: new ObjectId(userId),
                                                            source: { user: new ObjectId(userId), mailbox: msg.mailbox },
                                                            destination: { user: new ObjectId(userId), mailbox: targetMailboxId },
                                                            updates: [{ uid: msg.uid, message: msg._id }]
                                                        },
                                                        (err, result) => {
                                                            if (err) return reject(err);
                                                            resolve(result);
                                    ...
                                    // Legacy support for direct flag operations (deprecated, prefer keywords)
                                    if (patch.setFlags || patch.addFlags || patch.removeFlags) {
                                        needsUpdate = true;
                                        // Direct DB update for legacy support (not recommended)
                                        if (patch.setFlags) {
                        ...
                        if (args.create) {
                            for (let clientId of Object.keys(args.create)) {
                                ...
                                try {
                                    ...
                                    // Check if submitMessage is available (load-order dependency fix)
                                    if (!server.submitMessage || typeof server.submitMessage !== 'function') {
                                        notCreated[clientId] = {
                                            type: 'serverFail',
                                            description: 'Message submission not available. Submit API module not loaded.'
                                        };
                                        continue;
                                    }
                    ...
                    if (methodName === 'Email/send') {
                        ...
                        log.warn('JMAP', 'DEPRECATED: Email/send called instead of EmailSubmission/set. Client should use RFC 8621 EmailSubmission/set.');
                        
                        // Check if submitMessage is available
                        if (!server.submitMessage || typeof server.submitMessage !== 'function') {
                            responses.push(['error', {
                                type: 'serverFail',
                                description: 'Message submission not available. Submit API module not loaded.'

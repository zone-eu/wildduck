...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const addThreadCountersToMessageList = async (user, list, collection = 'messages', draftsInResponseOnly = false) => {
        const threadIdsToCount = list.map(message => message.thread);
        const responseDraftThreads = draftsInResponseOnly
            ? new Set(list.filter(message => message.draft).map(message => message.thread.toString()))
            : false;
        ...
        return list.map(message => {
            ...
            message.hasDrafts = draftsInResponseOnly ? responseDraftThreads.has(message.thread.toString()) : !!(matchingThreadCount && matchingThreadCount.hasDrafts);
            return message;
        });
    };
    ...
    server.get(
            ...
            validationObjs: {
                ...
                queryParams: {
                    ...
                    includeHeaders: Joi.alternatives()
                        ...
                        .description('Comma separated list of header keys to include in the response or true to return all headers')
                },
                response: {
                    200: {
                        description: 'Success',
                                ...
                                .items(
                                    Joi.object({
                                        ...
                                        hasDrafts: booleanSchema.description(
                                            'If true, then the Thread contains at least one draft. For non-collapsed results, only drafts in the response are considered. Included if threadCounters query argument was true'
                                        ),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (result.value.includeHeaders === true || result.value.includeHeaders === 'true') {
                includeHeaders = true;
            } else if (typeof result.value.includeHeaders === 'string') {
                includeHeaders = result.value.includeHeaders.split(',');
            }
            ...
            if (threadCounters) {
                listingWrapper.listing.results = await addThreadCountersToMessageList(user, listingWrapper.listing.results, 'messages', !collapseThreads);
            }
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (result.value.q) {
                ...
                filter = await getMongoDBQuery(db, user, result.value.q, { useAndSearch: result.value.useAndSearch });

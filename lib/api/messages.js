...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const getCollapsedListing = async (filter, { user, limit, projection, paginatedField, sortAscending, next, previous, maxTimeMS }) => {
        ...
        if (tuples.length) {
            ...
            // Check only the page's threads so the pagination pipeline can stay covered by its index.
                    ...
                    [
                        {
                            $match: {
                                user,
                                unseen: true,
                                $and: [filter, { thread: { $in: documents.map(messageData => messageData.thread) } }]
                            }
                        },
                        {
                            $group: { _id: '$thread' }
            ...
            for (const messageData of documents) {
                messageData.unseen = messageData.unseen || unseenThreadIds.has(messageData.thread.toString());
            }

...
const { acquireSearchDebounce } = require('../search-debounce');
...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const hasTextQuery = value => {
        ...
        return Object.values(value).some(entry => hasTextQuery(entry));
    };
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (hasTextQuery(filter)) {
                let debounce;
                try {
                    debounce = await acquireSearchDebounce(db.redis, user, config.api.search?.debounceMs);
                } catch (err) {
                    // Search must remain available if the protective Redis check fails.
                    log.error('API', 'Failed to check search debounce for user=%s error=%s', user, err.message);
                }

                if (debounce && !debounce.success) {
                    const retryAfter = Math.max(1, Math.ceil(debounce.retryAfterMs / 1000));
                    res.setHeader('Retry-After', retryAfter);
                    res.status(429);
                    return res.json({
                        error: 'Search was rate limited',

...
const { ensureKeywords, attachKeywordPaths } = require('../keyword-handler');
...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
                            ...
                            keywords: Joi.array()
                                .items(
                                    Joi.object({
                                        ...
                                        keyword: Joi.string().required().description('Keyword name (last path component)'),
    ...
    server.post(
        {
            ...
            validationObjs: {
                ...
                queryParams: {},
                requestBody: {
                    path: keywordSchema.required().description('Full keyword path, separated by /; at most 256 characters and 5 levels'),
                    sess: sessSchema,
                    ip: sessIPSchema
                },
                response: {
    ...
    server.put(
        {
            ...
            name: 'renameKeyword',
            description: 'Renames a keyword while preserving its stable ID and all message and filter assignments.',
            tags: ['Messages'],
            validationObjs: {
                requestBody: {
                    path: keywordSchema.required().description('New full keyword path, separated by /; at most 256 characters and 5 levels'),
                    sess: sessSchema,
                    ip: sessIPSchema
                },
                queryParams: {},
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const record = await collection.findOne(
                { _id: keyword, user },
                { projection: { path: 1, deleting: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
            );
            if (!record) {
                res.status(404);
                return res.json({
            ...
            const conflict = await collection.findOne(
                { user, path: newPath, _id: { $ne: keyword } },
                { projection: { path: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
            );
            if (conflict) {
                res.status(409);
                return res.json({
                    error: `Keyword ${newPath} already exists`,
            ...
            let renamed;
            try {
                renamed = await collection.findOneAndUpdate(
                    { _id: keyword, user, deleting: { $ne: true } },
                    { $set: { path: newPath, updated: new Date() } },
                    { returnDocument: 'after', projection: { path: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
                );
            } catch (err) {
                if (err.code === 11000) {
                    res.status(409);
    ...
    server.del(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            await db.database.collection('keywords').updateMany(
                { user, _id: { $in: keywordIds } },
    ...
    // Expands an uid query to cover sibling messages of the matched threads in every
    // mailbox belonging to the user. The result is grouped by mailbox because UIDs are
    // only unique within a mailbox.
    const expandMessageQueryToAllMailboxes = async (user, mailbox, messageQuery) => {
        const selectedMessages = await db.database
            .collection('messages')
            .find(
                {
        ...
        const selectedIdsWithoutThread = selectedMessages.filter(messageData => !messageData.thread).map(messageData => messageData._id);
        let query = { user };
        if (threadIds.length && selectedIdsWithoutThread.length) {
            query.$or = [{ thread: { $in: threadIds } }, { _id: { $in: selectedIdsWithoutThread } }];
        } else if (threadIds.length) {
            query.thread = { $in: threadIds };
        } else {
            query._id = { $in: selectedIdsWithoutThread };
        }

        const cursor = db.database
            .collection('messages')
            .find(query, {
                projection: {
                    mailbox: true,
        ...
        try {
            ...
            while ((messageData = await cursor.next())) {
                const mailboxId = messageData.mailbox.toString();
                let entry = byMailbox.get(mailboxId);
                if (!entry) {
                    entry = { mailbox: messageData.mailbox, uids: [] };
                    byMailbox.set(mailboxId, entry);
                }
                entry.uids.push(messageData.uid);
            }
        } finally {
            await cursor.close();
        }

        return Array.from(byMailbox.values()).map(entry => ({
            mailbox: entry.mailbox,
            messageQuery: tools.checkRangeQuery(entry.uids.sort((a, b) => a - b))
        }));
    };
    ...
    const putMessageHandler = async (req, res) => {
        ...
        const updateAllThreadMessages = result.value.updateThread && result.value.updateThreadAll;
        ...
        if (updateAllThreadMessages && result.value.moveTo) {
            res.status(400);
            return res.json({
                error: 'updateThreadAll can not be combined with moveTo',
                code: 'InputValidationError'
            });
        }
        ...
        let messageQueries;
        if (updateAllThreadMessages) {
            try {
                messageQueries = await expandMessageQueryToAllMailboxes(user, mailbox, messageQuery);
            } catch (err) {
                res.status(500);
                return res.json({
            ...
            messageQueries = [{ mailbox, messageQuery }];
        }
        ...
        let updated = 0;
        try {
            for (const entry of messageQueries) {
                updated += await updateMessage(user, entry.mailbox, entry.messageQuery, result.value);
            }
        } catch (err) {
            res.status(500); // TODO: use response code specific status
            return res.json({
    ...
    server.put(
        {
            ...
            validationObjs: {
                requestBody: {
                    ...
                    updateThreadAll: booleanSchema
                        .default(false)
                        .description(
                            'Only used when updateThread is true. If true, applies the requested updates to matching thread messages in all mailboxes. Can not be combined with moveTo'
                        ),
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                ...
                return res.json({
                    error: err.message,
                    code: err.responseCode ? err.code : err.imapResponse || 'InternalDatabaseError'
                });
            }

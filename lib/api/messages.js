...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const expandMessageQueryToThreads = async (user, mailbox, messageQuery) => {
        ...
        const threadIds = Array.from(
            new Set(
                selectedMessages
                    .filter(messageData => messageData.thread)
                    .map(messageData => messageData.thread.toString())
            )
        ).map(thread => new ObjectId(thread));

        if (!threadIds.length) {
            return messageQuery;
        }

        const threadMessages = await db.database
            .collection('messages')
            .find({
                user,
                mailbox,
        ...
        const messageUids = Array.from(new Set(selectedMessages.concat(threadMessages).map(messageData => messageData.uid))).sort(
            (a, b) => a - b
        );

        return messageUids.length === 1 ? messageUids[0] : { $in: messageUids };
    };
            ...
            validationObjs: {
                requestBody: {
                    ...
                    updateThread: booleanSchema
                        .default(false)
                        .description('If true, applies the requested updates to all messages in the selected message threads in this Mailbox'),

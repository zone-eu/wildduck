...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler, mailReadHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (mailReadHandler) {
                return res.json(
                    await mailReadHandler.bind(result.value.user).listMessages({
                        mailbox: result.value.mailbox,
                        unseen: result.value.unseen,
                        metaData: result.value.metaData,
                        threadCounters: result.value.threadCounters,
                        includeHasDrafts: result.value.includeHasDrafts,
                        collapseThreads: result.value.collapseThreads,
                        limit: result.value.limit,
                        maxLimit: 250,
                        order: result.value.order,
                        next: result.value.next,
                        previous: result.value.previous,
                        includeHeaders: result.value.includeHeaders,
                        includeBimi: true
                    })
                );
            }
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (mailReadHandler) {
                ...
                return res.json(
                    await mailReadHandler.bind(user).searchMessages({
                        ...result.value,
                        maxLimit: 250,
                        includeBimi: true,
                        onTiming: timing => {
                            server.loggelf({
                                short_message: timing.retry ? '[MESSAGES] Search listing retry' : '[MESSAGES] Search listing',
                                _mail_action: 'messages_search',
                                _is_retry: timing.retry || undefined,
                                _user: user,
                                _sess: result.value.sess,
                                _ip: result.value.ip,
                                _query_time: timing.elapsed
                            });
                        }
                    })
                );
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (mailReadHandler) {
                let user = new ObjectId(result.value.user);
                let mailbox = new ObjectId(result.value.mailbox);
                let response = await mailReadHandler.bind(user).getMessage({
                    mailbox,
                    uid: result.value.message,
                    includeSensitive: true,
                    replaceCidLinks: result.value.replaceCidLinks,
                    renderAttachmentUrl: attachment =>
                        server.router.render('attachment', {
                            user,
                            mailbox,
                ...
                if (result.value.markAsSeen && !response.seen) {
                    await updateMessage(user, mailbox, result.value.message, { seen: true });
                    response.seen = true;
                }

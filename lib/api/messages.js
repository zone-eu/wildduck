    ...
    const putMessageHandler = async (req, reply) => {
        ...
        if (moveTo) {
            ...
            try {
                ...
                // TODO: use response code specific status
                return reply.code(500).send({
                    error: err.message,
                    code: err.code
                });
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            if (attachmentData.transferEncoding === 'base64') {
                return reply.send(attachmentStream.pipe(new libbase64.Decoder()));
            } else if (attachmentData.transferEncoding === 'quoted-printable') {
                return reply.send(attachmentStream.pipe(new libqp.Decoder()));
            } else {
                if (!/ascii|utf[-_]?8/i.test(attachmentCharset) && values.sendAsString) {
                    return reply.send(attachmentStream.pipe(iconv.decodeStream(attachmentCharset)));
                }
                return reply.send(attachmentStream);
            }
        }

...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            res.charSet('utf-8');
            const { pathParams, requestBody } = req.route.spec.validationObjs;
            const result = Joi.object({ ...pathParams, ...requestBody }).validate(req.params, { abortEarly: false, convert: true });
            if (result.error) {
                res.status(400);
                return res.json({ error: result.error.message, code: 'InputValidationError', details: tools.validationErrors(result) });
            }
            req.validate(
                req.user && req.user === result.value.user ? roles.can(req.role).updateOwn('messages') : roles.can(req.role).updateAny('messages')
            );
            const user = new ObjectId(result.value.user);
            if (!(await db.users.collection('users').countDocuments({ _id: user }, { limit: 1 }))) {
                res.status(404);
                return res.json({ error: 'This user does not exist', code: 'UserNotFound' });
            }
            const path = result.value.path;
            const keywordResult = await ensureKeywords(db.database, user, [path]);
            const record = await db.database.collection('keywords').findOne({ user, path });
            ...
            return res.json({ success: true, id: record._id.toString(), path });
        })
    );

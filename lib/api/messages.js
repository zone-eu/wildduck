...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let attachmentMessageData;
            try {
                attachmentMessageData = messageData.attachments.find(att => att.id === attachment);
            } catch (err) {
                res.status(404);
                return res.json({
                    error: 'This attachment does not exist',
            ...
            res.writeHead(200, {
                'Content-Disposition': `attachment; filename="${attachmentMessageData.filename}"`,

...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (replaceCidLinks) {
                // restify-era code called server.router.render('attachment', ...) which
                // returned null (the route is named getMessageAttachment), so the links
                // were replaced with the string "null"; render the intended URL instead
                messageData.html = (messageData.html || []).map(html =>
                    html.replace(/attachment:([a-f0-9]+)\/(ATT\d+)/g, (str, mid, aid) =>
                        server.render('getMessageAttachment', { user, mailbox, message, attachment: aid })
                    )
                );

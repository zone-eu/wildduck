...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (req.user && req.user === result.value.user) {
                req.validate(
                    result.value.action && result.value.action.delete
                        ? roles.can(req.role).deleteOwn('messages')
                        : roles.can(req.role).updateOwn('messages')
                );
            } else {
                req.validate(
                    result.value.action && result.value.action.delete
                        ? roles.can(req.role).deleteAny('messages')
                        : roles.can(req.role).updateAny('messages')
                );
            }

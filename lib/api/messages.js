...
const { AddressOptionalNameArray, Header, Attachment, ReferenceWithAttachments, Bimi, AddressOptionalName } = require('../schemas/request/messages-schemas');
...
// Joi.string().trim().empty(''): no length cap (only the fulltext query field had max 255)
const shortTextQuery = description => ({ type: 'string', minLength: 1, wdTrim: true, wdEmpty: true, description });
...
// wdEmpty belongs next to anyOf, not inside a branch: the convert pre-pass only
// reads it at the level it walks, so an empty value would otherwise be a 400
// (Joi dropped the key through .empty(''))
const expiresSchema = description => ({
    wdEmpty: true,
    anyOf: [
        { wdType: 'date', wdInstanceof: 'Date' },
        { type: 'boolean', wdType: 'boolean' }
    ],
    description
});
...
const messageListingResponse = (title, itemResponse = messageListingItemResponse) => ({
    type: 'object',
    title,
    additionalProperties: true,
    properties: {
        ...
        results: { type: 'array', items: itemResponse, description: 'Message listing' }
    },
    required: ['success']
});

// archived messages are identified by their archive ObjectId, not a numeric
// mailbox uid; a numeric id type would make the serializer reject the listing
const archivedMessageListingItemResponse = Object.assign({}, messageListingItemResponse, {
    title: 'GetArchivedMessagesResult',
    properties: Object.assign({}, messageListingItemResponse.properties, {
        id: { description: 'ID of the archived Message' }
    })
});
...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    // expands the getMessageAttachment route pattern (restify-era code used the
    // router's render() for this)
    const attachmentUrl = (user, mailbox, message, attachment) =>
        `/users/${encodeURIComponent(user)}/mailboxes/${encodeURIComponent(mailbox)}/messages/${encodeURIComponent(message)}/attachments/${encodeURIComponent(
            attachment
        )}`;
    ...
    const putMessageHandler = async (req, reply) => {
        const values = req.params;

        if (values.metaData) {
            if (typeof values.metaData === 'object') {
                try {
                    values.metaData = JSON.stringify(values.metaData);
                } catch (err) {
                ...
                try {
                    let value = JSON.parse(values.metaData);
        ...
        let moveTo = values.moveTo ? new ObjectId(values.moveTo) : false;
        ...
        let updated;
        try {
            updated = await updateMessage(user, mailbox, messageQuery, values);
        } catch (err) {
            return reply.code(err.responseCode || 500).send({
                error: err.message,
                code: err.code
    ...
    server.route({
        ...
        config: {
            ...
            validationObjs: {
                ...
                queryParams: {
                    ...
                    // Joi.any().allow('asc', 'desc'): any value passes, non-'asc' sorts descending
                    order: { default: 'desc', wdEmpty: true, description: 'Ordering of the records by insert date' },
        ...
        async handler(req, reply) {
            ...
            let threadCounters = values.threadCounters;
            let collapseThreads = values.collapseThreads;
            let pageNext = values.next;
            let pagePrevious = values.previous;
            let sortAscending = values.order === 'asc';
            const filterUnseen = values.unseen;

            const includeHeaders = values.includeHeaders;
            ...
            let projection = getMessageListingProjection(values.metaData, includeHeaders);
            ...
            validationObjs: {
                queryParams: {
                    ...
                    ...{
                        ...
                        // Joi.any().allow('asc', 'desc'): any value passes
                        order: {
                            wdEmpty: true,
        ...
        async handler(req, reply) {
            ...
            if (values.q) {
                ...
                if (hasESFeatureFlag) {
                    ...
                    let searchQuery = await getElasticSearchQuery(db, user, values.q);
                ...
                filter = await getMongoDBQuery(db, user, values.q, { useAndSearch: values.useAndSearch, searchable: values.searchable });
                query = values.q;
            } else {
                let prepared = await prepareSearchFilter(db, user, values);
            ...
            const projection = getMessageListingProjection(values.metaData, includeHeaders);
    ...
    server.route({
                ...
                requestBody: {
                    ...
                    ...{
                        ...
                        action: {
                            ...
                            properties: {
                                moveTo: objectIdSchema('ID of the target Mailbox if you want to move messages'),
                            ...
                            allOf: [
                                {
                                    // the Joi custom validator rejected ANY other key
                                    // (known or unknown) next to delete: true
        ...
        async handler(req, reply) {
            ...
            // permissions check
            if (req.user && req.user === values.user) {
                req.validate(values.action && values.action.delete ? roles.can(req.role).deleteOwn('messages') : roles.can(req.role).updateOwn('messages'));
            } else {
                req.validate(values.action && values.action.delete ? roles.can(req.role).deleteAny('messages') : roles.can(req.role).updateAny('messages'));
            }
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let mailbox = new ObjectId(values.mailbox);
            let message = values.message;
            let replaceCidLinks = values.replaceCidLinks;
            ...
            if (replaceCidLinks) {
                // replace attachment links with the expanded getMessageAttachment
                // route URL (restify-era code called server.router.render('attachment', ...)
                // which returned null because the route is named getMessageAttachment,
                // so the links were replaced with the string "null")
                messageData.html = (messageData.html || []).map(html =>
                    html.replace(/attachment:([a-f0-9]+)\/(ATT\d+)/g, (str, mid, aid) => attachmentUrl(user, mailbox, message, aid))
                );

                messageData.text = messageData.text.replace(/attachment:([a-f0-9]+)\/(ATT\d+)/g, (str, mid, aid) => attachmentUrl(user, mailbox, message, aid));
            }

            if (values.markAsSeen && messageData.unseen) {
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            return reply.send(response.value);
        }
    });
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            const logStreamError = err => log.error('API', 'message=%s attachment=%s error=%s', messageData._id, attachment, err.message);

            reply.code(200).headers(headers);

            let decoder = null;
            if (decode) {
                if (attachmentData.transferEncoding === 'base64') {
                    decoder = new libbase64.Decoder();
                } else if (attachmentData.transferEncoding === 'quoted-printable') {
                    decoder = new libqp.Decoder();
                } else if (!/ascii|utf[-_]?8/i.test(attachmentCharset) && values.sendAsString) {
                    decoder = iconv.decodeStream(attachmentCharset);
                }
            }

            if (!decoder) {
                attachmentStream.once('error', logStreamError);
                return reply.send(attachmentStream);
            }

            // .pipe() forwards neither errors nor teardown, and fastify only
            // watches the stream it was handed, so a mid transfer GridFS
            // failure would leave the reply hanging with the socket, the cursor
            // and the reply context leaked (the restify code ended the response
            // by hand). pipeline() propagates in both directions, so a client
            // that disconnects also destroys the GridFS read stream
            pipeline(attachmentStream, decoder, err => {
                if (err) {
                    logStreamError(err);
                }
            });

            return reply.send(decoder);
        }
    });
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            if (values.async) {
                ...
                try {
                    r = await taskHandler.ensure('clear-folder', { user, mailbox }, { user, mailbox, skipArchive: values.skipArchive });
                } catch (err) {
                    return reply.code(500).send({
                        error: 'MongoDB Error: ' + err.message,
                        code: 'InternalDatabaseError'
                ...
                while ((messageData = await cursor.next())) {
                    ...
                    try {
                        await messageHandler.delAsync({
                            ...
                            mailbox: { user, mailbox },
                            messageData,
                            archive: !messageData.flags.includes('\\Draft') && !values.skipArchive
                        });
                        deleted++;
                    } catch (err) {
                        errors++;
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let replacePrevious = values.replacePrevious;
            ...
            let referencedMessage = await getReferencedMessage(userData, values);

            if (referencedMessage) {
                if (['reply', 'replyAll'].includes(values.reference.action) && referencedMessage.inReplyTo) {
                ...
                values.draft = true; // only draft messages can reference to another message
            }
            ...
            let data = {
                from: values.from || { name: userData.name, address: userData.address },
                date,
                replyTo: values.replyTo,
                to: values.to ? values.to.filter(toObj => toObj.address !== '') : undefined,
                cc: values.cc,
                bcc: values.bcc,
                subject: values.subject || referencedMessage.subject,
                text: values.text,
                html: values.html,
                headers: extraHeaders.concat(values.headers || []),
                attachments: extraAttachments.concat(values.attachments || []),
            ...
            if (!data.to && !envelope.to.length && referencedMessage && ['reply', 'replyAll'].includes(values.reference.action)) {
            ...
            if ((userData.encryptMessages || mailboxData.encryptMessages) && !values.draft) {
            ...
            if (values.bimi) {
                try {
                    let bimiRecord = await bimiHandler.fetchByDomain(values.bimi.domain, values.bimi.selector);
                    if (bimiRecord) {
                        verificationResults = {
                            bimi: bimiRecord._id
                        };
                    ...
                    log.error('API', 'BIMIFAIL domain=%s selector=%s error=%s', values.bimi.domain, values.bimi.selector || '', err.message);
                }
            }
            ...
            try {
                const resp = await messageHandler.addAsync({
                    ...
                    meta: {
                        source: 'API',
                        from: '',
                        origin: values.ip || '127.0.0.1',
                        transtype: 'UPLOAD',
                        time: date,
                        custom: values.metaData || '',
                        reference: referencedMessage
                            ? {
                                  action: values.reference.action,
                                  mailbox: values.reference.mailbox,
                                  id: values.reference.id
                              }
                            : false,
                        envelope,
                        files
                    ...
                    verificationResults,
                    flags: []
                        .concat('unseen' in values ? (values.unseen ? [] : '\\Seen') : [])
                        .concat('flagged' in values ? (values.flagged ? '\\Flagged' : []) : [])
                        .concat('draft' in values ? (values.draft ? '\\Draft' : []) : []),
                    raw,
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            [].concat(values.addresses || []).forEach(address => {
                forwardTargets.push({ type: 'mail', value: address });
            });

            if (messageData.forwardTargets) {
                if (values.target) {
                    forwardTargets = forwardTargets.concat(messageData.forwardTargets[values.target - 1] || []);
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            let mailbox = new ObjectId(values.mailbox);
            let message = values.message;
            let deleteFiles = values.deleteFiles;
            let sendTime = values.sendTime;
            ...
            for (const to of Array.isArray(envelope.to) ? envelope.to : [envelope.to]) {
                server.loggelf({
                    short_message: `[RCPT TO: ${to}] ${values.sess}`,
        ...
        config: {
            ...
            validationObjs: {
                ...
                response: {
                    200: {
                        description: 'Success',
                        model: messageListingResponse('GetArchivedMessagesResponse', archivedMessageListingItemResponse)
                    }
                }
            }
        },
    ...
    server.route({
        ...
        config: {
            ...
            validationObjs: {
                requestBody: {
                    mailbox: objectIdSchema('ID of the target Mailbox. If not set then original mailbox is used.'),
                    sess: sessSchema,
                    ip: sessIPSchema
                },
                queryParams: {},
                pathParams: {
                    user: userId,
                    // archived messages are identified by their archive
                    // ObjectId; the numeric uid schema used here before made
                    // every real archive id fail validation
                    message: objectIdSchema('Archived message ID', { wdRequired: true })
                },
                response: {
                    200: {
                        description: 'Success',
        ...
        async handler(req, reply) {
            ...
            let user = new ObjectId(values.user);
            let message = new ObjectId(values.message);
            let mailbox = values.mailbox ? new ObjectId(values.mailbox) : false;

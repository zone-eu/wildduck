...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const getCollapsedListing = async (filter, { limit, projection, paginatedField, sortAscending, next, previous, maxTimeMS, includeTotal }) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let total = collapseThreads ? undefined : await getFilteredMessageCount(filter);
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const totalPromise = collapseThreads
                ? false
                : getFilteredMessageCount(filter).then(
                      total => ({ total }),
                      error => ({ error })
                  );
            ...
            let opts = {
                limit,
                // The retry removes the mailbox constraint, so keep its filter separate
                // from the concurrently running count query.
                query: { ...filter },
                fields: {
            ...
            const getSearchListing = maxTimeMS => {
                ...
                return getCollapsedListing(opts.query, {
                    limit,
            ...
            try {
                ...
                listingWrapper = await getSearchListing(collapseThreads ? consts.DB_MAX_TIME_MESSAGES : consts.DB_MAX_TIME_MESSAGES_SEARCH);
                ...
                if (!collapseThreads && err.code === 50 && err.codeName === 'MaxTimeMSExpired') {
                    try {

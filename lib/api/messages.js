...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const getCollapsedListing = async (filter, { user, limit, projection, paginatedField, sortAscending, next, previous, maxTimeMS }) => {
        ...
        if (tuples.length) {
            ...
            // Unread representatives already establish thread state, and a seen-only filter cannot match unread siblings.
            const messagesToCheck = documents.filter(messageData => !messageData.unseen);
            if (filter.unseen !== false && messagesToCheck.length) {
                // Check only the page's remaining threads so the pagination pipeline can stay covered by its index.
                // Keep the listing filter: unread messages in other mailboxes or outside a search must not affect these results.
                const unseenThreads = await db.database
                    .collection('messages')
                    .aggregate(
                        [
                            {
                                $match: {
                                    user,
                                    unseen: true,
                                    $and: [filter, { thread: { $in: messagesToCheck.map(messageData => messageData.thread) } }]
                                }
                            },
                            {
                                $group: { _id: '$thread' }
                    ...
                    .toArray();
                const unseenThreadIds = new Set(unseenThreads.map(thread => thread._id.toString()));
                for (const messageData of messagesToCheck) {
                    messageData.unseen = unseenThreadIds.has(messageData.thread.toString());
                }
            }
                                ...
                                .items(
                                    Joi.object({
                                        ...
                                        seen: booleanSchema
                                            .required()
                                            .description(
                                                'Is this message already seen. For collapsed results, true if all matching messages in the thread are seen'
                                            ),

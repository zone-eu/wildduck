...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (result.value.includeHeaders === true) {
                includeHeaders = true;
            ...
            let total = await getFilteredMessageCount(filter, user);
    ...
    server.del(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                const mailboxKey = `${tools.redisHashTag(db.redis, user.toString())}:${mailbox}`;
                // clear counters
                await db.redis.multi().del(`total:${mailboxKey}`).del(`unseen:${mailboxKey}`).exec();
            } catch (err) {
                // ignore
            }
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let limitCheck;
            try {
                limitCheck = await messageHandler.counters.asyncTTLCounter(
                    `wdr:${tools.redisHashTag(messageHandler.redis, userData._id.toString())}`,
                    0,
                    maxRecipients,
                    false
                );
    ...
    async function getFilteredMessageCount(filter, user) {
        if (Object.keys(filter).length === 1 && filter.mailbox) {
            // Try to use cached value to get the count
            return await getMailboxCounter(db, filter.mailbox, false, user);
        }

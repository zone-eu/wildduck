...
const { ensureKeywords, attachKeywordPaths, expandPaths } = require('../keyword-handler');
const { keywordPathSchema } = require('../schemas/request/keywords-schemas');
...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const getUserKeywords = tools.getUserKeywords;
    ...
    const addThreadCountersToMessageList = async (
        ...
        {
            collection = 'messages',
            includeThreadMessageCount = true,
            includeThreadUnseen = false,
            includeThreadFlagged = false,
            includeHasDrafts = false,
            matchDraftReferences = false
        } = {}
    ) => {
        ...
        return list.map(message => {
            ...
            if (includeThreadUnseen) {
                message.unseen = !!(matchingThreadCount && matchingThreadCount.unseen);
            }

            if (includeThreadFlagged) {
                message.flagged = !!(matchingThreadCount && matchingThreadCount.flagged);
            }
    ...
    server.get(
        {
            path: '/users/:user/keywords',
            summary: 'Get user keywords',
            name: 'getUserKeywords',
            description: 'Lists persistent keyword paths, including empty labels, with optional exact-path message counters',
            validationObjs: {
                requestBody: {},
                pathParams: {
                    ...
                    200: {
                        ...
                        model: Joi.object({
                            ...
                            keywords: Joi.array()
                                .items(
                                    Joi.object({
                                        id: Joi.string().required().description('ID of the keyword'),
                                        keyword: Joi.string().required().description('Keyword name'),
                                        path: Joi.string().required().description('Full keyword path'),
                                        name: Joi.string().required().description('Last component of the keyword path'),
                                        total: Joi.number().description('Total number of messages with this keyword'),
                                        unseen: Joi.number().description('Unseen number of messages with this keyword')
                                    })
                                )
                                .required()
                        }).$_setFlag('objectName', 'UserKeywordsResponse')
                    }
                }
            },
            tags: ['Messages']
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const keywords = await getUserKeywords(db, user, result.value.counters);
    ...
    server.post(
        {
            ...
            name: 'createKeyword',
            description: 'Creates a persistent keyword and its parent paths. Repeated creation returns the existing keyword.',
            tags: ['Messages'],
            validationObjs: {
                pathParams: { user: userId },
                queryParams: {},
                requestBody: {
                    path: keywordPathSchema.required().description('Full keyword path, separated by /; at most 256 characters and 5 levels'),
                    sess: sessSchema,
                    ip: sessIPSchema
                },
                response: {
                    200: {
                        ...
                        model: Joi.object({
                            ...
                            id: Joi.string().required().description('ID of the keyword'),
                            path: Joi.string().required().description('Full keyword path')
                        }).$_setFlag('objectName', 'CreateKeywordResponse')
                    }
                }
            }
        },
        tools.responseWrapper(async (req, res) => {
            ...
            const keywordResult = await ensureKeywords(db.database, user, [path]);
            const record = keywordResult.keywords[0];
            const parentPaths = keywordResult.created.filter(keywordPath => keywordPath !== path);
            server.loggelf({
                short_message: keywordResult.created.includes(path) ? '[KEYWORDS] Created keyword' : '[KEYWORDS] Existing keyword requested',
                _mail_action: 'keyword_create',
                _user: user.toString(),
                _keyword: path,
                _keyword_id: record._id.toString(),
                _keyword_created: keywordResult.created.includes(path),
                _keyword_parents: parentPaths,
    ...
    server.put(
        {
            path: '/users/:user/keywords/:keyword',
            summary: 'Rename a keyword',
            name: 'renameKeyword',
            description: 'Renames a keyword and its descendants while preserving their stable IDs and all message and filter assignments.',
            tags: ['Messages'],
            validationObjs: {
                requestBody: {
                    path: keywordPathSchema.required().description('New full keyword path, separated by /; at most 256 characters and 5 levels'),
                    sess: sessSchema,
                    ip: sessIPSchema
                },
                queryParams: {},
                pathParams: {
                    user: userId,
                    keyword: Joi.string().hex().lowercase().length(24).required().description('ID of the keyword')
                },
                response: {
                    200: {
                        description: 'Success',
                        model: Joi.object({
                            success: booleanSchema.required(),
                            id: Joi.string().required().description('ID of the keyword'),
                            path: Joi.string().required().description('New full keyword path'),
                            oldPath: Joi.string().required().description('Previous full keyword path')
                        }).$_setFlag('objectName', 'RenameKeywordResponse')
                    }
                }
            }
        },
        tools.responseWrapper(async (req, res) => {
            ...
            const user = new ObjectId(result.value.user);
            const keyword = new ObjectId(result.value.keyword);
            const userExists = await db.users.collection('users').countDocuments({ _id: user }, { limit: 1 });
            if (!userExists) {
                res.status(404);
                return res.json({
            ...
            const collection = db.database.collection('keywords');
            const record = await collection.findOne(
                { _id: keyword, user },
                { projection: { path: 1, pendingPath: 1, deleting: 1, renaming: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
            );
            if (!record) {
                res.status(404);
                return res.json({
            ...
            if (record.deleting) {
                res.status(409);
                return res.json({
                    error: 'This keyword is being deleted',
                    code: 'KeywordDeleting'
                });
            }
            ...
            if (record.renaming) {
                res.status(409);
                return res.json({
                    error: 'This keyword is being renamed',
                    code: 'KeywordRenaming'
                });
            }

            const newPath = result.value.path;
            ...
            // a keyword cannot be renamed into a path nested inside itself
            if (newPath.startsWith(record.path + '/')) {
                res.status(409);
                return res.json({
                    error: 'A keyword cannot be renamed to a path nested inside itself',
                    code: 'KeywordConflict'
                });
            }

            const moving = await collection
                .find(
                    { user, path: { $regex: `^${tools.escapeRegexStr(record.path)}(?:/|$)` } },
                    {
                        projection: { path: 1, pendingPath: 1, deleting: 1, renaming: 1 },
                        maxTimeMS: consts.DB_MAX_TIME_MAILBOXES
                    }
                )
                .toArray();
            if (moving.some(entry => entry.deleting)) {
                res.status(409);
                return res.json({
                    error: 'This keyword or one of its descendants is being deleted',
                    code: 'KeywordDeleting'
                });
            }

            if (moving.some(entry => entry.renaming)) {
                res.status(409);
                return res.json({
                    error: 'This keyword or one of its descendants is being renamed',
                    code: 'KeywordRenaming'
                });
            }

            const mappings = moving.map(entry => ({
                id: entry._id,
                oldPath: entry.path,
                newPath: entry.path === record.path ? newPath : newPath + entry.path.slice(record.path.length)
            }));
            const oldPaths = mappings.map(mapping => mapping.oldPath);
            const newPaths = mappings.map(mapping => mapping.newPath);
            const movingIds = moving.map(entry => entry._id);

            // Validate every descendant destination, not only the requested root.
            expandPaths(newPaths);

            // The user/path and user/pendingPath indexes make exact collisions
            // cheap. The anchored subtree lookup also prevents merging into an
            // existing destination subtree.
            const destinationRegex = { $regex: `^${tools.escapeRegexStr(newPath)}(?:/|$)` };
            const conflict = await collection.findOne(
                {
                    user,
                    _id: { $nin: movingIds },
                    $or: [
                        { path: destinationRegex },
                        { pendingPath: destinationRegex },
                        { path: { $in: newPaths } },
                        { pendingPath: { $in: newPaths } }
                    ]
                },
                { projection: { path: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
            );
            if (conflict) {
                res.status(409);
                return res.json({
                    error: `Keyword ${conflict.path || newPath} already exists at or below the new path`,
                    code: 'KeywordConflict'
                });
            }

            // the new path cannot land inside a subtree that is being deleted or renamed
            const newPrefixes = [];
            let prefix = newPath;
            while ((prefix = prefix.split('/').slice(0, -1).join('/'))) {
                newPrefixes.push(prefix);
            }
            if (newPrefixes.length) {
                const blockedAncestor = await collection.findOne(
                    {
                        user,
                        path: { $in: newPrefixes },
                        $or: [{ deleting: true }, { renaming: true }]
                    },
                    { projection: { deleting: 1, renaming: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
                );
                if (blockedAncestor) {
                    res.status(409);
                    return res.json({
                        error: blockedAncestor.deleting
                            ? 'The new path is inside a keyword that is being deleted'
                            : 'The new path is inside a keyword that is being renamed',
                        code: blockedAncestor.deleting ? 'KeywordDeleting' : 'KeywordRenaming'
                    });
                }
                await ensureKeywords(db.database, user, newPrefixes);
            }

            const renamedAt = new Date();
            const renameOps = mappings.map(mapping => ({
                updateOne: {
                    filter: { _id: mapping.id, user, path: mapping.oldPath, deleting: { $ne: true }, renaming: { $ne: true } },
                    update: { $set: { pendingPath: mapping.newPath, renaming: true, renamingAt: renamedAt } },
                    upsert: false
                }
            }));
            const rollbackRename = () =>
                collection.bulkWrite(
                    mappings.map(mapping => ({
                        updateOne: {
                            filter: { _id: mapping.id, user, renaming: true },
                            update: {
                                $set: { path: mapping.oldPath, pendingPath: mapping.oldPath },
                                $unset: { renaming: true, renamingAt: true }
                            }
                        }
                    })),
                    { ordered: false, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
                );
            let renameResult;
            try {
                renameResult = await collection.bulkWrite(renameOps, { ordered: true, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES });
            } catch (err) {
                await rollbackRename();
                if (err.code === 11000) {
                    res.status(409);
                    return res.json({
                        error: 'A keyword with the new path already exists',
                        code: 'KeywordConflict'
                    });
                }
                throw err;
            }
            if (!renameResult || renameResult.matchedCount !== mappings.length) {
                await rollbackRename();
                throw new Error('Failed to mark keyword paths for rename');
            }

            let finalized;
            try {
                finalized = await collection.bulkWrite(
                    mappings.map(mapping => ({
                        updateOne: {
                            filter: { _id: mapping.id, user, pendingPath: mapping.newPath, renaming: true },
                            update: {
                                $set: { path: mapping.newPath, pendingPath: mapping.newPath, updated: new Date() },
                                $unset: { renaming: true, renamingAt: true }
                            }
                        }
                    })),
            ...
            if (!finalized || finalized.matchedCount !== mappings.length) {
                await rollbackRename();
                throw new Error('Failed to finalize keyword rename');
            }

            server.loggelf({
                short_message: '[KEYWORDS] Renamed keyword',
                _mail_action: 'keyword_rename',
                _user: user.toString(),
                _keyword: record.path,
                _keyword_id: keyword.toString(),
                _new_path: newPath,
                _old_paths: oldPaths,
                _new_paths: newPaths,
                _keywords_updated: finalized.modifiedCount || 0,
                _sess: result.value.sess,
                _ip: result.value.ip || req.remoteAddress
            });
    ...
    server.del(
        {
            ...
            name: 'deleteKeyword',
            description: 'Asynchronously removes the keyword, its descendants, and their assignments from messages and filters.',
            tags: ['Messages'],
            validationObjs: {
                pathParams: {
                    user: userId,
                ...
                response: {
                    200: {
                        ...
                        model: Joi.object({
                            success: booleanSchema.required(),
                            scheduled: Joi.string().required().description('ID of the keyword deletion task'),
                            existing: booleanSchema.required().description('True if an existing deletion task was reused')
                        }).$_setFlag('objectName', 'DeleteKeywordResponse')
                    }
                }
            }
        },
        tools.responseWrapper(async (req, res) => {
            ...
            const user = new ObjectId(result.value.user);
            const keyword = new ObjectId(result.value.keyword);
            const record = await db.database.collection('keywords').findOne({ _id: keyword, user });
            if (!record) {
                res.status(404);
                return res.json({
                    error: 'This keyword does not exist',
            ...
            const paths = await db.database
                .collection('keywords')
                .find(
                    { user, path: { $regex: `^${tools.escapeRegexStr(record.path)}(?:/|$)` } },
                    { projection: { path: 1, renaming: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
                )
                .toArray();
            if (paths.some(entry => entry.renaming)) {
                res.status(409);
                return res.json({
                    error: 'This keyword has descendants that are being renamed',
                    code: 'KeywordRenaming'
                });
            }
            const keywordPaths = paths.map(entry => entry.path);
            const keywordIds = paths.map(entry => entry._id);
            const marked = await db.database.collection('keywords').updateMany(
                { user, _id: { $in: keywordIds }, renaming: { $ne: true } },
                { $set: { deleting: true, deletingAt: new Date() } },
                { maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
            );
            if (marked.matchedCount !== keywordIds.length) {
                await db.database
                    .collection('keywords')
                    .updateMany({ user, _id: { $in: keywordIds } }, { $unset: { deleting: true, deletingAt: true } });
                res.status(409);
                return res.json({
                    error: 'This keyword has descendants that are being renamed',
                    code: 'KeywordRenaming'
            ...
            let task;
            try {
                task = await taskHandler.ensure(
                    'keyword-delete',
                    { user, keyword },
                    { user, keyword, path: record.path, paths: keywordPaths, ids: keywordIds }
                );
            } catch (err) {
                await db.database.collection('keywords').updateMany({ user, _id: { $in: keywordIds } }, { $unset: { deleting: true, deletingAt: true } });
                throw err;
            }

            server.loggelf({
                short_message: '[KEYWORDS] Scheduled keyword deletion',
                _mail_action: 'keyword_delete',
                _task_id: task.task.toString(),
                _user: user.toString(),
                _keyword: record.path,
                _keyword_id: keyword.toString(),
                _keyword_paths: keywordPaths,
                _task_existing: task.existing,
                _sess: result.value.sess,
                _ip: result.value.ip || req.remoteAddress
            });
            return res.json({
                success: true,
                scheduled: task.task.toString(),
    ...
    server.get(
        {
            ...
            validationObjs: {
                ...
                pathParams: {
                    user: Joi.string().hex().lowercase().length(24).required().description('ID of the User'),
                    keyword: keywordSchema.required().description('Keyword to count')
                },
                queryParams: {
                    sess: sessSchema,
                    ip: sessIPSchema
    ...
    server.get(
                                ...
                                .items(
                                    Joi.object({
                                        ...
                                        seen: booleanSchema
                                            .required()
                                            .description(
                                                'Is this message already seen. For collapsed results, true if all messages in the thread are seen'
                                            ),
                                        ...
                                        flagged: booleanSchema
                                            .required()
                                            .description(
                                                'Does this message have a Flagged flag. For collapsed results, true if any message in the thread is flagged'
                                            ),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (threadCounters || includeHasDrafts || collapseThreads) {
                listingWrapper.listing.results = await addThreadCountersToMessageList(user, listingWrapper.listing.results, {
                    ...
                    includeThreadUnseen: collapseThreads,
                    includeThreadFlagged: collapseThreads,
                    includeHasDrafts,
            ...
            await attachKeywordPaths(db.database, user, listingWrapper.listing.results);
    ...
    const searchSchema = {
        ...
        keyword: keywordSchema.trim().empty('').description('If set, then matches only messages that have this custom keyword set'),
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            await attachKeywordPaths(db.database, user, [messageData]);
            ...
            let response = {
                ...
                keywords: messageData.keywordPaths || [],
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                const keywordResult = await ensureKeywords(db.database, user, result.value.keywords || []);
                const resp = await messageHandler.addAsync({
                    ...
                    keywords: keywordResult.keywords.map(keyword => keyword._id),
                    raw,

...
module.exports = (db, server, mailboxHandler, mailReadHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (mailReadHandler) {
                return res.json(
                    await mailReadHandler.bind(user).listMailboxes({
                        specialUse: result.value.specialUse,
                        showHidden: result.value.showHidden,
                        counters: result.value.counters,
                        sizes: result.value.sizes
                    })
                );
            }

    ...
    function submitMessage(options, callback) {
        ...
        db.users.collection('users').findOne(
            ...
            (err, userData) => {
                ...
                settingsHandler
                    .getMulti(['const:max:storage', 'const:max:recipients', 'const:max:forwards', 'const:max:rcpt_to'])
                    .then(settings => {
                        ...
                        let updateReferencedMessage = done => {
                            ...
                            db.database.collection('messages').findOneAndUpdate(
                                ...
                                (err, r) => {
                                    if (err) {
                                        // The submitted message is already queued and stored; reference flags are best-effort.
                                        return done();
                                    }
                            ...
                            validateFromAddress(tools.normalizeAddress(envelope.from.address), (err, address) => {
                                ...
                                validateFromAddress(tools.normalizeAddress(options.from.address), (err, address) => {
                                    ...
                                    let addToDeliveryQueue = next => {
                                        if (!compiledEnvelope.to || !compiledEnvelope.to.length || options.uploadOnly || options.isDraft) {
                                        ...
                                        const recipientCount = compiledEnvelope.to.length;
                                    ...
                                    addToDeliveryQueue((err, outbound) => {
                                        ...
                                        if (overQuota) {
                                            ...
                                            if (outbound) {
                                                return updateReferencedMessage(() => callback(null, response));
                                            }
                                        ...
                                        (async () => {
                                            ...
                                            try {
                                                if (userData.encryptMessages && !options.isDraft) {
                                                    let encryptResult = await messageHandler.encryptMessageAsync(tools.getUserEncryptionKey(userData), {
                                            ...
                                            let meta = {
                                                ...
                                                reference: reference
                                                    ? {
                                                          ...
                                                          mailbox:
                                                              options.reference.mailbox && options.reference.mailbox.toString
                                                                  ? options.reference.mailbox.toString()
                                                                  : options.reference.mailbox,
                                            ...
                                            messageHandler.add(messageOptions, (err, success, info) => {
                                                ...
                                                let done = () => callback(null, response);
                                                ...
                                                if (outbound) {
                                                    return updateReferencedMessage(deleteDraft);
                                                }
    ...
    server.post(
        {
            ...
            validationObjs: {
                requestBody: {
                    ...
                    isDraft: booleanSchema.default(false).description('If true then stores the message as a draft without queueing delivery'),
                ...
                response: {
                    200: {
                        ...
                        model: Joi.object({
                            success: successRes,
                            message: Joi.object({
                                mailbox: Joi.string().required().description('Mailbox ID the message was stored to or false if not stored'),
                                id: Joi.number().description('Message ID in the Mailbox or false if not stored').required(),
                                queueId: Joi.string().required().description('Queue ID in MTA or false if not queued')
                            })
                                .required()
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            if (result.value.meta && typeof result.value.meta === 'string') {
                try {
                    result.value.meta = JSON.parse(result.value.meta);
                } catch (err) {
                    result.value.meta = undefined;
                }
            }

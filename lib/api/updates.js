...
const getJournalPayload = entry => {
    ...
    Object.keys(entry).forEach(key => {
        if (!['_id', 'ignore', 'user', 'modseq', 'unseenChange', 'created', 'counterKeywords', 'flaggedCounterInvalidated'].includes(key)) {
...
function loadJournalStream(db, res, user, lastEventId, done, onEntry) {
    ...
    let invalidatedKeywords = new Set();
    ...
    let emitKeywordCounters = async next => {
        ...
        try {
            const trackedKeywords = new Set(await tools.getTrackedKeywords(db.redis, user));
            const toEmit = [...changedKeywords].filter(keyword => invalidatedKeywords.has(keyword) || trackedKeywords.has(keyword));
            if (!toEmit.length) {
                return next();
            }
        ...
        cursor.next((err, e) => {
            ...
            switch (e.command) {
                ...
                case 'FETCH':
                    ...
                    break;
                case 'DELETE':
                    for (const keyword of e.counterKeywords ?? []) {
                        changedKeywords.add(keyword);
                        invalidatedKeywords.add(keyword);
                    }

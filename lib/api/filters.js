...
module.exports = (db, server, userHandler, settingsHandler) => {
    const attachFilterKeywordPaths = async filters => {
        filters = [].concat(filters || []);
        const ids = [];
        const seen = new Set();
        for (const filterData of filters) {
            for (const keyword of filterData.action?.keywords || []) {
                const id = keyword.toString();
                if (ObjectId.isValid(id) && !seen.has(id)) {
                    seen.add(id);
                    ids.push(keyword);
                }
            }
        ...
        const keywords = await db.database.collection('keywords').find({ _id: { $in: ids } }, { projection: { path: 1 } }).toArray();
        const pathsById = new Map(keywords.map(keyword => [keyword._id.toString(), keyword.path]));
        for (const filterData of filters) {
            if (filterData.action?.keywords) {
                filterData.action.keywords = filterData.action.keywords
                    .map(keyword => pathsById.get(keyword.toString()))
                    .filter(path => path);
            }
        }
        return filters;
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            await attachFilterKeywordPaths(listingWrapper.listing.results);
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            await attachFilterKeywordPaths(filters);
    ...
    server.get(
                ...
                response: {
                    200: {
                        ...
                        model: Joi.object({
                            success: successRes,
                            id: filterId,
                            name: Joi.string().description('Name for the filter'),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            await attachFilterKeywordPaths([filterData]);
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let r;
            const filterKeywordPaths = filterData.action.keywords || [];
            try {
                const keywordResult = await ensureKeywords(db.database, user, filterKeywordPaths);
                if (filterKeywordPaths.length) {
                    filterData.action.keywords = keywordResult.keywords.map(keyword => keyword._id);
                }
                r = await db.database.collection('filters').insertOne(filterData);
            } catch (err) {
                res.status(err.responseCode || 500);
                return res.json({
                    error: err.responseCode ? err.message : 'MongoDB Error: ' + err.message,
                    code: err.responseCode ? err.code : 'InternalDatabaseError'
                });
            }
            ...
            const filterStrings = getFilterStrings({
                ...filterData,
                action: { ...filterData.action, keywords: filterKeywordPaths }
            });
    ...
    server.put(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let r;
            try {
                const keywordResult = await ensureKeywords(db.database, user, $set['action.keywords'] || []);
                if ($set['action.keywords']) {
                    $set['action.keywords'] = keywordResult.keywords.map(keyword => keyword._id);
                }

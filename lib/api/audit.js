...
module.exports = (db, server, auditHandler) => {
                        ...
                        model: {
                            ...
                            properties: {
                                ...
                                import: {
                                    ...
                                    // failed and copied are only present once the import
                                    // task starts counting (lib/tasks/audit.js creates the
                                    // subdocument with just a status)
                                }
                            },
                            // expires and import are echoed from the db document and may be
                            // absent on audits created before the import status tracking
                            required: ['success', 'id', 'user']
                        }
                    }
                }
            }
    ...
    server.route({
        ...
        async handler(req, reply) {
            ...
            output.on('error', err => {
                // fastify owns the response once the stream is handed to
                // reply.send() and tears it down on error; ending reply.raw
                // here as well would race its own error handling

...
async function userHasAddressInDomain(db, user, address) {
    if (!address || address.includes('*')) {
        return false;
    }

    const atPos = address.lastIndexOf('@');
    const domain = atPos >= 0 ? address.substring(atPos + 1) : '';
    if (!domain) {
        return false;
    }

    const existingAddress = await db.users.collection('addresses').findOne(
        {
            user,
            address: new RegExp(`^[^*]+@${escapeRegex(domain)}$`)
        },
        {
            projection: {
                _id: true
...
module.exports = (db, server, userHandler, settingsHandler) => {
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const ownAddressAssignment = !!(req.user && req.user === result.value.user);
            const permission = ownAddressAssignment ? roles.can(req.role).createOwn('addresses') : roles.can(req.role).createAny('addresses');
            req.validate(permission);
            ...
            if (wcpos >= 0) {
                if (ownAddressAssignment) {
                    res.status(403);
                    return res.json({
                        error: 'Wildcard addresses require elevated privileges',
            ...
            if (ownAddressAssignment) {
                let domainAllowed;
                try {
                    domainAllowed = await userHasAddressInDomain(db, user, address);
                } catch (err) {
                    res.status(500);
                    return res.json({
                        error: 'MongoDB Error: ' + err.message,
                ...
                if (!domainAllowed) {
                    res.status(403);
                    return res.json({
                        error: 'Address domain is not assigned to this user',
                        code: 'AddressDomainNotAllowed'
                    });
                }
            }
    ...
    server.put(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            const ownAddressAssignment = !!(req.user && req.user === result.value.user);
            const permission = ownAddressAssignment ? roles.can(req.role).updateOwn('addresses') : roles.can(req.role).updateAny('addresses');
            req.validate(permission);
            result.value = permission.filter(result.value);
            ...
            if (ownAddressAssignment && updates.address) {
                let domainAllowed;
                try {
                    domainAllowed = await userHasAddressInDomain(db, user, updates.address);
                } catch (err) {
                    res.status(500);
                    return res.json({
                        error: 'MongoDB Error: ' + err.message,

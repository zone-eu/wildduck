...
class FilterHandler {
    ...
    async storeMessage(userData, options) {
        ...
        let encryptMessage = async () => {
            ...
            try {
                let encryptResult = await this.messageHandler.encryptMessageAsync(tools.getUserEncryptionKey(userData), {
                    chunks,
                    chunklen
                });
        ...
        let forwardMessage = async () => {
            ...
            const userKey = tools.redisHashTag(this.messageHandler.redis, userData._id.toString());
            try {
                const counterResult = await this.ttlcounter(`wdf:${userKey}`, forwardTargets.size, userData.forwards || consts.MAX_FORWARDS, false);
                if (!counterResult.success) {
                    log.silly('Filter', 'FRWRDFAIL key=%s error=%s', `wdf:${userKey}`, 'Precondition failed');
                ...
                log.info('Filter', 'FRWRDFAIL key=%s error=%s', `wdf:${userKey}`, err.message);

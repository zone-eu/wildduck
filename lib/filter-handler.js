...
class FilterHandler {
    ...
    async storeMessage(userData, options) {
        ...
        let sourceMessageId = [].concat((prepared.mimeTree && prepared.mimeTree.parsedHeader && prepared.mimeTree.parsedHeader['message-id']) || []).pop();
        if (sourceMessageId && prepared.msgid && prepared.hdate) {
            try {
                let targetMailboxData = await this.messageHandler.getMailboxAsync({
                    user: userData._id,
                    [mailboxQueryKey]: mailboxQueryValue,
                    inboxDefault: true
                });

...
class FilterHandler {
    ...
    async storeMessage(userData, options) {
        ...
        const findHeaderIndex = (name, pos) => {
            const headerName = getHeaderName(name);
            if (!headerName) {
                return -1;
            }
            ...
            let seen = 0;
            for (let i = 0; i < prepared.mimeTree.header.length; i++) {
                if (getHeaderKey(prepared.mimeTree.header[i]) === headerName.toLowerCase() && ++seen === pos) {
                    return i;
                }
            }
        ...
        overrideHeaders.forEach(change => {
            ...
            switch (change.action) {
                ...
                case 'change': {
                    ...
                    const pos = Math.trunc(Number(change.pos));
                    const index = pos > 0 ? findHeaderIndex(change.name, pos) : -1;
                    if (index < 0) {
                        prepared.mimeTree.header.push(headerLine);
                        visibleHeaders.push(headerLine);
                        headersChanged = true;
                ...
                case 'delete': {
                    ...
                    const index = findHeaderIndex(change.name, pos);
                    if (index >= 0) {

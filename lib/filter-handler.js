...
const replaceRawHeaders = (source, headers) => {
    ...
    if (headerEnd >= 0) {
        return Buffer.concat([headerBlock, source.subarray(headerEnd + 4)]);
    }

    headerEnd = source.indexOf(Buffer.from('\n\n'));
    if (headerEnd >= 0) {
        return Buffer.concat([headerBlock, source.subarray(headerEnd + 2)]);
    }
...
class FilterHandler {
    ...
    async storeMessage(userData, options) {
        ...
        if (options.mimeTree) {
            reusableMimeTree = structuredClone(options.mimeTree);
            if (options.maildata) {
                reusableMaildata = options.maildata;
            } else {
                reusableMaildata = this.messageHandler.indexer.getMaildata(reusableMimeTree);
            }
            if (reusableMimeTree && reusableMimeTree.header) {
                // Backwards compatibility for callers that cached an already decorated mimeTree
                if (/^Delivered-To/.test(reusableMimeTree.header[0])) {
                    reusableMimeTree.header.shift();
                }
                if (/^Return-Path/.test(reusableMimeTree.header[0])) {
                    reusableMimeTree.header.shift();
                }
            }
            prepared = await this.messageHandler.prepareMessageAsync({
                mimeTree: structuredClone(reusableMimeTree)
            });
        } else {
            ...
            reusableMimeTree = structuredClone(prepared.mimeTree);
        }
        ...
        let encryptMessage = async () => {
            ...
            try {
                let encryptionRaw = replaceRawHeaders(raw || Buffer.concat(chunks, chunklen), prepared.mimeTree.header);

...
const spamActionSourceMap = {
    domainaccess: 'user-domainaccess',
    spamLevel: 'user-spamlevel',
    filter: 'user-filter'
};

const formatClassificationDate = value => {
...
const getClassificationInfo = (source, isSpam, details) => {
    switch (source) {
        case 'domainaccess': {
            const domain = details?.domainaccess?.domain;
            const domainText = domain ? `the sender domain ${domain}` : 'the sender domain';
            if (isSpam) {
                return `This message was moved to Junk because messages from ${domainText} are blocked for your account. If this is not spam, mark it as Not Junk.`;
            }

            return `This message was marked as not junk because messages from ${domainText} are allowed for your account. If this is spam, move it to Junk.`;
        }

        case 'spamLevel':
            if (isSpam) {
                return 'This message was moved to Junk by spam protection rules. If this is not spam, mark it as Not Junk.';
            }

            return 'This message was marked as not junk by spam protection rules. If this is spam, move it to Junk.';

        case 'filter': {
            const created = formatClassificationDate(details?.filter?.created);
            if (isSpam) {
                return `This message was moved to Junk by your mail filter added on ${created}. If this is not spam, update your filter settings in Webmail.`;
            }

            return `This message was marked as not junk by your mail filter added on ${created}. If this is spam, update your filter settings in Webmail.`;
        }
...
class FilterHandler {
    ...
    async storeMessage(userData, options) {
        ...
        const removeHeader = name => {
            const headerName = getHeaderName(name);
            if (!headerName || !Array.isArray(prepared.mimeTree.header)) {
                return false;
            }

            let removed = false;
            const headerKey = headerName.toLowerCase();
            for (let i = prepared.mimeTree.header.length - 1; i >= 0; i--) {
                if (getHeaderKey(prepared.mimeTree.header[i]) !== headerKey) {
                    continue;
                }

                const visibleIndex = visibleHeaders.indexOf(prepared.mimeTree.header[i]);
                if (visibleIndex >= 0) {
                    visibleHeaders.splice(visibleIndex, 1);
                }

                prepared.mimeTree.header.splice(i, 1);
                removed = true;
            }
        ...
        let spamActionDomainaccess = false;
        let spamActionFilter = false;
                    ...
                    if (domainaccessData) {
                        switch (domainaccessData.action) {
                            case 'block':
                                ...
                                spamActionDomainaccess = domainaccessData;
        ...
        for (let filterData of filters) {
            ...
            Object.keys(filterData.action).forEach(key => {
                ...
                if (key === 'keywords') {
                    ...
                    filterActions.set('keywords', [
                        ...new Map([...existingKeywords, ...newKeywords].map(keyword => [keyword.toString(), keyword])).values()
                    ]);
                    return;
                }
        ...
        if (!spamOverrideApplied && spamActionSource !== false) {
            const isSpam = filterActions.get('spam') === true;
            const classificationInfo = getClassificationInfo(spamActionSource, isSpam, {
                domainaccess: spamActionDomainaccess,
                filter: spamActionFilter
            });

            const replace = true;
            addHeader('WD-Mail-Classification', isSpam ? 'junk' : 'not-junk', { replace });
            addHeader('WD-Mail-Classification-Source', spamActionSourceMap?.[spamActionSource] || 'unknown', { replace });
            addHeader('WD-Mail-Classification-Info', classificationInfo, { replace });
        }
        await refreshPrepared();
        ...
        const encryptMessage = async () => {
            ...
            try {
                const sourceRaw =
                    raw ||
                    (Array.isArray(chunks) && Buffer.concat(chunks, chunklen)) ||
                    (await rebuildMimeTree(this.messageHandler.indexer, prepared.mimeTree));
        ...
        const messageKeywords = filterActions.get('keywords') || [];
        const keywordRecords = messageKeywords.length
            ? await this.db.database
                  .collection('keywords')
                  .find({ user: userData._id, _id: { $in: messageKeywords }, deleting: { $ne: true } }, { projection: { path: 1 } })
                  .toArray()
            : [];
        const keywordPathsById = new Map(keywordRecords.map(keyword => [keyword._id.toString(), keyword.path]));
            ...
            switch (key) {
                ...
                case 'keywords':
                    if (Array.isArray(value) && value.length) {
                        filterResults.push({ keywords: value.map(keyword => keywordPathsById.get(keyword.toString())).filter(path => path) });
                    }
                    break;
                case 'mailbox':
                    if (value) {

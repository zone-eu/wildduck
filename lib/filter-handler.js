...
async function checkFilter(filterData, prepared, maildata) {
    ...
    if (query.text) {
        const { andTerms, orTerms } = tools.parseFilterQueryText(query.text);
        ...
        const andMatches = !andTerms.length || andTerms.every(term => tools.filterQueryTermMatches(normalizedEmailText, term));

        const orMatches = !orTerms.length || orTerms.some(term => tools.filterQueryTermMatches(normalizedEmailText, term));

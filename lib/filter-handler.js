...
async function checkFilter(filterData, prepared, maildata) {
    ...
    if (query.text) {
        const { andTerms, orTerms } = parseFilterQueryText(query.text);
        ...
        const andMatches = !andTerms.length || andTerms.every(term => filterQueryTermMatches(normalizedEmailText, term));

        const orMatches = !orTerms.length || orTerms.some(term => filterQueryTermMatches(normalizedEmailText, term));
...
function parseFilterQueryText(queryText) {
    ...
    const { cleanQuery, phrases: exactPhrases } = extractQuotedPhrases(queryText);

    const orParts = cleanQuery.split(' OR '); // Has to be uppercase
...
function filterQueryTermMatches(text, term, exactPhrases = []) {
    ...
    if (phraseMatch) {
        ...
        if (exactPhrase) {
            return text.includes(exactPhrase.trim());
        }
        return false;
    }

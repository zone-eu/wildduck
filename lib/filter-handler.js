...
const replaceRawHeaders = (source, headers) => {
    source = Buffer.isBuffer(source) ? source : Buffer.from(source || '', 'binary');

    let headerBlock = Buffer.from([].concat(headers || []).join('\r\n') + '\r\n\r\n', 'binary');
    let headerEnd = source.indexOf(Buffer.from('\r\n\r\n'));
    let headerEndLen = 4;

    let lfHeaderEnd = source.indexOf(Buffer.from('\n\n'));
    ...
    if (headerEnd >= 0) {
        return Buffer.concat([headerBlock, source.subarray(headerEnd + headerEndLen)]);
    }

    return Buffer.concat([headerBlock, source]);
};
...
const cloneMimeTree = mimeTree => cloneValue(mimeTree);
...
const cloneMimeTreeRoot = mimeTree => {
    ...
    let cloned = Object.assign({}, mimeTree);
    cloned.header = [].concat(mimeTree.header || []);
    cloned.parsedHeader = cloneValue(mimeTree.parsedHeader || {});
...
const parseRootHeaders = headers => {
    let mimeTree = parseMimeTree(Buffer.from([].concat(headers || []).join('\r\n') + '\r\n\r\n', 'binary'));
    return (mimeTree && mimeTree.parsedHeader) || {};
};

const concatChunks = (chunks, chunklen) => {
...
const rebuildMimeTree = async (indexer, mimeTree) => {
    let outputStream = indexer.rebuild(mimeTree);
    ...
    async storeMessage(userData, options) {
        ...
        if (options.mimeTree) {
            ...
            prepared = await this.messageHandler.prepareMessageAsync({
                mimeTree: cloneMimeTree(reusableMimeTree)
            });
        } else {
        ...
        let refreshPrepared = async () => {
            let preparedId = prepared.id;
            let preparedIdate = prepared.idate;
            let preparedFlags = prepared.flags;

            prepared.mimeTree.parsedHeader = parseRootHeaders(prepared.mimeTree.header);
            prepared = await this.messageHandler.prepareMessageAsync({
        ...
        // will change headers, move them, add new, replace etc
        await plugins.handler.runHooks('message:wd_headers', [prepared.mimeTree.header, prepared.mimeTree.parsedHeader, meta?.overrides]);
        ...
        let encryptMessage = async () => {
            ...
            try {
                let sourceRaw = raw || concatChunks(chunks, chunklen) || (await rebuildMimeTree(this.messageHandler.indexer, prepared.mimeTree));
                let encryptionRaw = replaceRawHeaders(sourceRaw, prepared.mimeTree.header);
                let encryptResult = await this.messageHandler.encryptMessageAsync(tools.getUserEncryptionKey(userData), encryptionRaw);
                ...
                if (encryptResult) {
                    ...
                    prepared = await this.messageHandler.prepareMessageAsync({
                        raw: Buffer.concat([extraHeader, encryptResult.raw])
                    });

...
class UserHandler {
        ...
        if (data.featureFlags && Object.keys(data.featureFlags).length) {
            ...
            for (let featureFlag of Object.keys(data.featureFlags)) {
                if (data.featureFlags[featureFlag]) {
                    req.push(this.redis.sadd(`feature:${featureFlag}`, user.toString()));
                }
            }
            try {
    ...
    async update(user, data) {
        ...
        let slotKeys = new Set(['iup', 'idw', 'pdw', 'pxm']);
        let flushKeys = [];
        let flushHKeys = [];
        ...
        Object.keys(data).forEach(key => {
            ...
            if (resetKeys.has(key)) {
                let prefix = resetKeys.get(key);
                let keySuffix = slotKeys.has(prefix) ? tools.redisClusterValue(user, this.redis) : user.toString();
                flushKeys.push(`${prefix}:${keySuffix}`);
            }
        ...
        if (flushKeys.length || flushHKeys.length) {
            ...
            try {
                ...
                flushKeys.forEach(key => {
                    flushreq.push(this.redis.del(key));
                });

                flushHKeys.forEach(entry => {
                    flushreq.push(this.redis.hdel(entry.key, entry.value));
                });

                await Promise.all(flushreq);
            } catch (err) {
                // ignore
            }
        }
            ...
            if (existingAccount.featureFlags && Object.keys(existingAccount.featureFlags).length) {
                ...
                for (let featureFlag of Object.keys(existingAccount.featureFlags)) {
                    if (existingAccount.featureFlags[featureFlag]) {
                        req.push(this.redis.srem(`feature:${featureFlag}`, user.toString()));
                    }
                }
                try {
                    await Promise.all(req);

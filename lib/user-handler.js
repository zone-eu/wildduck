...
const originalAsyncAuthenticate = UserHandler.prototype.asyncAuthenticate;
UserHandler.prototype.asyncAuthenticate = async function (username, password, requiredScope, meta) {
    meta = meta || {};
    let scope = requiredScope || 'master';
    let service = meta.protocol || (['imap', 'pop3'].includes((scope || '').toString().toLowerCase()) ? scope : 'api');

    try {
        let result = await originalAsyncAuthenticate.call(this, username, password, requiredScope, meta);

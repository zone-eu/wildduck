...
class UserHandler {
    ...
    async generatePending2faNonce(user, data) {
        ...
        const pending2faData = {
            ...
            methods: JSON.stringify([].concat(data.methods || [])),
    ...
    async getPending2faAuth(user, twoFactorNonce, method, options = {}) {
        ...
        try {
            pending2faData = await this.redis.hgetall(pending2faKey);
        } catch (err) {
            log.error('Redis', 'REDISFAIL id=%s error=%s', user, err.message);
        }
        ...
        return parsePending2faData(user, pending2faKey, pending2faData, method, options);
    }
    ...
    async checkTotp(user, data) {
        ...
        let consumedTotpNonce;
        let pending2faAuth = false;
        if (data.pending2fa) {
            pending2faAuth = await this.consumePending2faAuth(user, data.totpNonce, 'totp', { code: 'InvalidTotpNonce' });
            consumedTotpNonce = pending2faAuth;
        } else {
            consumedTotpNonce = await this.validateTotpNonce(user, data.totpNonce, data.accessTokenHash);
        }
        ...
        if (!verified) {
            try {
                if (pending2faAuth) {
                    await this.restorePending2faAuth(consumedTotpNonce);
                } else {
    ...
    async webauthnGetAuthenticationOptions(user, data) {
        ...
        if (data.twoFactorNonce) {
            await this.getPending2faAuth(user, data.twoFactorNonce, 'webauthn');
        }
        ...
        try {
            await this.redis.multi().hmset(challengeKey, challengeData).expire(challengeKey, consts.WEBAUTHN_CHALLENGE_TTL).exec();
        } catch (err) {
            log.error('DB', 'REDISFAIL id=%s error=%s', user, err.message);
            throw err;
        }
    ...
    async webauthnAssertAuthentication(user, data) {
        ...
        let twoFactorNonce = challengeData.twoFactorNonce || data.twoFactorNonce;
...
function getPending2faKey(user, twoFactorNonce) {
    return `pending2fa:${user}:${crypto
        .createHash('sha256')
        .update(twoFactorNonce || '')
        .digest('hex')}`;
}
...
function parsePending2faData(user, key, data, method, options = {}) {
    ...
    methods = [].concat(methods || []).filter(entry => typeof entry === 'string');

...
class UserHandler {
    ...
    async asyncAuthenticate(username, password, requiredScope, meta) {
        ...
        if (config.pwned && config.pwned.enabled && config.pwned.type && config.pwned.type !== 'none') {
            try {
                isPasswordPwned = this.checkPwnedPasswordForUser(password);
    ...
    async checkPwnedPasswordForUser(password, ttl = 86400) {
        const cachePrefix = 'pwned:';
        ...
        const cacheKey = `${cachePrefix}${prefix}`;

        let cachedData;
        try {
            cachedData = await this.redis.get(cacheKey);

            const { found: count } = await tools.checkPwnedPassword(password, cachedData);
            ...
            // ignore
            // if redis cache is down default to API
        }

        if (!cachedData) {
            // no cached data
            const { found: count, lines } = await tools.checkPwnedPassword(password);
            // if error throw upstream

            if (lines && lines.length > 0) {
                await this.redis.set(cacheKey, ttl, lines); // cache result for future use
            }

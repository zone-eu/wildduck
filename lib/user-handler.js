    ...
    async update(user, data) {
        ...
        Object.keys(data).forEach(key => {
            ...
            if (resetKeys.has(key)) {
                ...
                flushKeys.push(`${prefix}:${tools.redisHashTag(this.redis, user.toString())}`);
            }
            if (key === 'imapMaxConnections') {
                flushHKeys.push({ key: `lim:imap:${tools.redisHashTag(this.redis, user.toString())}`, value: user.toString() });
            }

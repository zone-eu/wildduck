...
class UserHandler {
    ...
    async asyncAuthenticate(username, password, requiredScope, meta) {
        ...
        if (config.pwned && config.pwned.enabled && config.pwned.type && config.pwned.type !== 'none') {
            try {
                isPasswordPwned = await this.checkPwnedPasswordForUser(password);
    ...
    async checkPwnedPasswordForUser(password, ttl = 86400) {
        ...
        const { count, lines } = await tools.checkPwnedPassword(password, cachedData);
        // if error throw upstream

        if (lines && lines.length > 0 && !cachedData) {
            await this.redis.set(cacheKey, lines, 'EX', ttl); // cache result for future use
        }

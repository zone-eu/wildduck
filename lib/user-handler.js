    ...
    async create(data) {
        ...
        if (!data.emptyAddress) {
            ...
            try {
                ...
                this.users.collection('domaincache').insertOne({ domain: addressData.address.split('@')[1] });
            } catch (err) {
                try {
                    // try to rollback
                    await this.users.collection('users').deleteOne({ _id: user });
    ...
    async restore(user, options) {
        ...
        for (let address of accountDeleted.addresses || []) {
            try {
                ...
                this.users.collection('domaincache').insertOne({ domain: address.address.split('@')[1] });

...
class UserHandler {
    ...
    async webauthnAttestateRegistration(user, data) {
        ...
        // hgetall answers an empty object, not null, for a missing key
    ...
    async webauthnRemove(user, credential, data) {
        ...
        try {
            r = await this.users.collection('users').findOneAndUpdate(
                {
                    _id: user,
                    // without this the update matches even when the credential
                    // does not exist, and a bogus id reports success
        ...
        if (!r || !r.value) {
            const userExists = await this.users.collection('users').findOne({ _id: user }, { projection: { _id: true } });
            let err = new Error(userExists ? 'Could not find credential data' : 'Could not find user data');
            err.responseCode = 404;
            err.code = userExists ? 'CredentialNotFound' : 'UserNotFound';
            throw err;
        }
    ...
    async update(user, data) {
        ...
        // suspending or disabling an account must also lock out whatever is
        // already authenticated: existing sessions and issued API tokens
        ...
        Object.keys(data).forEach(key => {
            ...
            if (['suspended', 'disabled'].includes(key) && data[key]) {
                accessRevoked = true;
            }
        ...
        if (passwordChanged || accessRevoked) {
            if (!updateQuery.$inc) {
                updateQuery.$inc = {};
            }
            updateQuery.$inc.authVersion = 1;

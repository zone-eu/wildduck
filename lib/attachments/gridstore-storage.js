...
class GridstoreStorage {
        ...
        if (FEATURE_DECODE_ATTACHMENTS && attachment.transferEncoding === 'base64' && this.decodeBase64) {
            ...
            if (lineLen && lineLen <= 998) {
                ...
                if (attachment.lineCount >= expectedLineCount - 1 && attachment.lineCount <= expectedLineCount + 1) {
                    ...
                    if (Number.isFinite(attachment.lineCount)) {
                        // ensure lineCount is a number, 0 is acceptable value
                        metadata.lineCount = attachment.lineCount;
                    }
                }
            }
        }
    ...
    createReadStream(id, attachmentData, options) {
        ...
        let streamOptions = {};
        let tailSlice = false;
        let mainRange = { start: 0, length: 0 };

        let useDecoded = attachmentData && attachmentData.metadata && attachmentData.metadata.decoded && attachmentData.metadata.lineLen;
        const storedLineCount = normalizeLineCount(attachmentData && attachmentData.metadata && attachmentData.metadata.lineCount);

        if (attachmentData && attachmentData.metadata) {
            encoderOptions.lineLength = attachmentData.metadata.lineLen;

            if (useDecoded) {
                const requestedStart = Math.max(Number(options.startFrom) || 0, 0);
                const requestedLength = Math.max(Number(options.maxLength) || 0, 0);

                const encodedSize = Number(attachmentData.metadata.esize) || 0;
                const storedLineCountValue = storedLineCount;

                const base64CharLength = Math.ceil(attachmentData.length / 3) * 4;

                // If lineCount is missing, try to recover it from the stored esize
                let recoveredLineCount = null;
                if (storedLineCount === null && encodedSize) {
                    const diff = encodedSize - base64CharLength;
                    if (diff >= 0) {
                        recoveredLineCount = Math.floor(diff / 2);
                    }
                }

                const defaultLineBreaks = Math.max(0, Math.floor((base64CharLength - 1) / attachmentData.metadata.lineLen));
                const effectiveLineBreaks =
                    storedLineCountValue !== null
                        ? storedLineCountValue
                        : recoveredLineCount !== null
                        ? Math.max(defaultLineBreaks, recoveredLineCount)
                        : defaultLineBreaks;
                const trailingBreaks = Math.max(0, effectiveLineBreaks - defaultLineBreaks);

                const mainOutputLength = base64CharLength + defaultLineBreaks * 2;
                const totalOutputLength = base64CharLength + effectiveLineBreaks * 2;

                const rangeEnd = requestedLength ? Math.min(requestedStart + requestedLength, totalOutputLength) : totalOutputLength;

                const mainStart = Math.min(requestedStart, mainOutputLength);
                const mainEnd = Math.min(rangeEnd, mainOutputLength);
                const mainLength = Math.max(0, mainEnd - mainStart);
                mainRange = { start: mainStart, length: mainLength };

                if (trailingBreaks) {
                    // Preserve optional trailing empty lines from the original base64 payload
                    const tailStart = Math.max(requestedStart, mainOutputLength);
                    const tailEnd = Math.max(tailStart, Math.min(rangeEnd, totalOutputLength));
                    const tailLength = Math.max(0, tailEnd - tailStart);

                    if (tailLength) {
                        const tailOffset = tailStart - mainOutputLength;
                        const tailBuffer = Buffer.from('\r\n'.repeat(trailingBreaks));
                        tailSlice = tailBuffer.slice(tailOffset, tailOffset + tailLength);
                    }
                }

                if (mainLength) {
                    let offsetOptions = base64Offset(attachmentData.metadata.lineLen, mainStart, mainLength);
        ...
        if (useDecoded && !mainRange.length && tailSlice) {
            const passthrough = new Transform({
                transform(chunk, enc, cb) {
                    this.outputBytes = (this.outputBytes || 0) + chunk.length;
                    cb(null, chunk);
                }
            });
            passthrough.outputBytes = 0;

            setImmediate(() => {
                passthrough.outputBytes += tailSlice.length;
                passthrough.write(tailSlice);
                passthrough.end();
            });
        ...
        if (useDecoded) {
            ...
            const finalize = () => {
                if (tailSlice && tailSlice.length) {
                    output.outputBytes += tailSlice.length;
                    output.write(tailSlice);
                }
                output.end();
            };

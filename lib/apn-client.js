...
// upper bound on how long a queued push may be coalesced; without it, continuous mail
// traffic keeps resetting the per-event debounce timer and the push is never delivered
const APN_MAX_DEBOUNCE_DELAY = 10000;
...
class ApnClient {
    ...
    notify(user, mailboxId) {
        ...
        if (!this._pending.has(userId)) {
            ...
            this._pending.set(userId, { user, mailboxIds: new Map(), firstQueuedAt: Date.now() });
        }
        ...
        // Coalesce bursts, but cap the total wait: once the max window has elapsed the delay
        // clamps to 0 so a steady stream of notifications can no longer postpone delivery.
        let elapsed = Date.now() - entry.firstQueuedAt;
        let delay = Math.max(0, Math.min(APN_DEBOUNCE_DELAY, APN_MAX_DEBOUNCE_DELAY - elapsed));

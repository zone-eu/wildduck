...
const { isDomainValid } = require('@hapi/address');
...
const validateDomain = async (domain, certHandler) => {
    // check domain name format, TLD must be on the IANA list like with
    // the Joi.string().domain() validation this replaced
    if (typeof domain !== 'string' || !isDomainValid(domain, { tlds: { allow: tlds } })) {
...
const acquireCert = async (domain, acmeOptions, certificateData, certHandler) => {
    ...
    try {
        ...
        if (!acmeAccount) {
            log.info('ACME', 'Skip certificate renewal for %s, acme account not found', domain);
            return false;
        }

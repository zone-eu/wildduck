...
/**
 * Applies the access level's field allowlist from config/roles.json.
 *
 * This is what makes the `mcp:read` role the single declaration of what an agent may see,
 * rather than a second hand-written allowlist in the tool layer that can drift from it.
 *
 * @param {String} role Access level of the calling token.
 * @param {String} resource Resource name as used in config/roles.json.
 * @param {Object|Array} data Response data.
 * @returns {Object|Array} Data with unlisted fields removed.
 */
function filterFields(role, resource, data) {
    let permission = roles.can(role).readOwn(resource);
    if (!permission.granted) {
        throw createError('Not enough privileges', 'MissingPrivileges', 403);
    }
    // accesscontrol's filter handles an array in one pass, so a listing resolves the grant
...
class McpReader {
    ...
    async getAccount() {
        ...
        let user = filterFields(this.role, 'users', userData);
        let results = filterFields(this.role, 'addresses', addresses.results || []);
        let primary = (addresses.results || []).find(address => address.main);

        return {
            id: user.id,
            username: user.username || '',
            name: user.name || '',
            primaryAddress: (primary && primary.address) || user.address || '',
            aliases: results.filter(address => address.address !== ((primary && primary.address) || user.address)),
            quota: {
    ...
    async listMailboxes(options) {
        ...
        return filterFields(this.role, 'mailboxes', response.results || []);
    }
    ...
    async resolveMailbox(reference) {
        ...
        let mailbox = await this.get(`/mailboxes/${isId ? reference.toLowerCase() : 'resolve'}`, isId ? {} : { path: reference });

        return filterFields(this.role, 'mailboxes', mailbox);
    }
    ...
    listing(response) {
        return {
            ...
            messages: filterFields(this.role, 'messages', response.results || [])
        };
    }
    ...
    async getMessage({ mailbox, message }) {
        let response = await this.get(`/mailboxes/${mailbox}/messages/${message}`, {
        ...
        return filterFields(this.role, 'messages', response);
    }
}

...
function redisClusterValue(value, redis) {
    value = value === null || typeof value === 'undefined' ? '' : value.toString();
    if (!value || !redis || !redis.isCluster) {
        return value;
    }
    // strip existing braces to avoid nested hash tags
...
async function getMailboxCounter(db, mailbox, type) {
    const mailboxKey = redisClusterValue(mailbox, db.redis);
    const cacheKey = `${type || 'total'}:${mailboxKey}`;

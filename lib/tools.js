...
function checkRangeQuery(uids, ne, isContiguous = false) {
    ...
    // uids are sorted ascending; check for a single unbroken run
    let contiguous = isContiguous;
    if (!contiguous) {
        contiguous = true;
    ...
    if (contiguous) {
        ...
        return !ne ? range : { $not: range };
    }

    // Dense set (few gaps): bound it with a range and exclude the gaps instead of enumerating
    // every uid into a huge $in. { $gte, $lte, $nin } matches the same uids but scans the index
    // once rather than seeking per value. Bail to $in once gaps outnumber the uids (sparse set).
    if (!ne) {
        let gaps = [];
        let dense = true;
        for (let i = 1, len = uids.length; i < len && dense; i++) {
            for (let missing = uids[i - 1] + 1; missing < uids[i]; missing++) {
                gaps.push(missing);
                if (gaps.length > uids.length) {
                    dense = false;
                    break;
                }
            }
    ...
    // sparse set, or a negated match whose range form would need disjoint ranges
    return {
        [!ne ? '$in' : '$nin']: uids
    };
}
...
function getMessageRetentionState(mailboxData, addedAt = Date.now()) {
    ...
    return {
        retention,
        exp: true,
        rdate: addedAt + retention
    };
}

function applyMessageRetention(messageData, mailboxData, addedAt = Date.now()) {
    const retentionState = getMessageRetentionState(mailboxData, addedAt);

    messageData.retentionTime = addedAt;
    messageData.retention = retentionState.retention;
    messageData.exp = retentionState.exp;

    if ('rdate' in retentionState) {
        messageData.rdate = retentionState.rdate;
    } else {
...
function extractKeywords(flags) {
    return [...new Set((flags ?? []).filter(flag => typeof flag === 'string' && flag && !flag.startsWith('\\') && !consts.SYSTEM_FLAGS.has(flag)))];
}

const getAccountCounterVersionKey = user => `account-counters:{${user.toString()}}:version`;
const getTrackedKeywordsKey = user => `account-counters:{${user.toString()}}:keywords`;

async function getAccountCounterVersion(redis, user) {
    return (await redis.get(getAccountCounterVersionKey(user))) || '0';
}

async function getVersionedAccountValue(db, user, name, loader, deserialize = value => Number(value), serialize = value => String(value)) {
    const versionKey = getAccountCounterVersionKey(user);

    for (let attempt = 0; attempt < 3; attempt++) {
        const version = await getAccountCounterVersion(db.redis, user);
        const cacheKey = `account-counters:{${user.toString()}}:${version}:${name}`;
        ...
        const stored = await db.redis.setversionedvalue(versionKey, cacheKey, version, serialize(value), consts.KEYWORD_COUNTER_TTL);
        if (stored) {
            return value;
        }
    }
...
async function trackKeywords(redis, user, keywords) {
    const values = extractKeywords(keywords);
    if (!values.length) {
        return;
    }

    const key = getTrackedKeywordsKey(user);
    await redis.multi().sadd(key, ...values).expire(key, consts.KEYWORD_COUNTER_TTL).exec();
}

async function getTrackedKeywords(redis, user) {
    return redis.smembers(getTrackedKeywordsKey(user));
}

async function bumpAccountCounterVersion(redis, user) {
    const key = getAccountCounterVersionKey(user);
    await redis.set(key, crypto.randomBytes(12).toString('hex'), 'EX', consts.KEYWORD_COUNTER_TTL);
}

async function getUserMailboxIds(db, user) {
    const mailboxes = await db.database
        ...
        .toArray();
    return mailboxes.map(mailbox => mailbox._id);
}

async function getKeywordCounter(db, user, keyword, type) {
    try {
        await trackKeywords(db.redis, user, [keyword]);
        const keywordData = await db.database.collection('keywords').findOne(
            { user, path: keyword, deleting: { $ne: true } },
            { projection: { _id: true }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
        );
        if (!keywordData) {
            return 0;
        }
        return await getKeywordCounterById(db, user, keywordData._id, type);
    } catch (err) {
        errors.notify(err);
        throw err;
    }
...
async function getKeywordCounterById(db, user, keyword, type) {
    try {
        return await getVersionedAccountValue(db, user, `keyword:${type || 'total'}:${keyword.toString()}`, async () => {
            const mailboxIds = await getUserMailboxIds(db, user);
            ...
            return db.database.collection('messages').countDocuments(query, { maxTimeMS: consts.DB_MAX_TIME_MESSAGES_SEARCH });
        });
    } catch (err) {
        errors.notify(err);
        throw err;
...
async function getFlaggedCounter(db, user, type) {
    try {
        return await getVersionedAccountValue(db, user, `flagged:${type || 'total'}`, async () => {
            const mailboxIds = await getUserMailboxIds(db, user);
            if (!mailboxIds.length) {
                return 0;
            }
...
async function getUserKeywords(db, user, counters = false) {
    const records = await db.database
        .collection('keywords')
        .find({ user, deleting: { $ne: true } }, { projection: { path: 1 }, maxTimeMS: consts.DB_MAX_TIME_MAILBOXES })
        .sort({ path: 1 })
        .toArray();
    const keywords = [];
    // Keep counter queries bounded instead of starting two queries per label
    // simultaneously. The common names-only request never reads messages/Redis.
    for (const record of records) {
        const entry = {
            id: record._id.toString(),
            keyword: record.path.split('/').pop(),
            path: record.path
        };
        if (counters) {
            await trackKeywords(db.redis, user, [record.path]);
            [entry.total, entry.unseen] = await Promise.all([
                getKeywordCounterById(db, user, record._id),
                getKeywordCounterById(db, user, record._id, 'unseen')
            ]);
        }
        keywords.push(entry);
    }
...
function getUserEncryptionKey(userData) {
    if (userData.smimeCerts && userData.smimeCerts.length) {
        return {
            type: 'smime',
            certs: userData.smimeCerts,
            cipher: userData.smimeCipher || consts.SMIME_DEFAULT_CIPHER,
            keyTransport: userData.smimeKeyTransport || consts.SMIME_DEFAULT_RSA_KEY_TRANSPORT
        };
    }
    if (userData.pubKey) {
...
function getEnabled2fa(enabled2fa) {
    ...
    if (list.includes('u2f')) {
        ...
        listSet.delete('u2f');
        listSet.add('webauthn'); // legacy U2F entries are handled through WebAuthn endpoints
    ...
    responseWrapper(middleware) {
        return async (req, res) => {
            ...
            try {
                ...
                metrics.recordApiError(req.method, (req.route && req.route.path) || 'unknown', err.code || err.responseCode || 'error');

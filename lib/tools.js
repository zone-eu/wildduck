...
async function getUserKeywords(db, user, counters = true) {
    try {
        const keywords = await getVersionedAccountValue(
            db,
            user,
            counters ? 'keywords:with-counters' : 'keywords',
            async () => {
                const mailboxIds = await getUserMailboxIds(db, user);
                if (!mailboxIds.length) {
                    return [];
                                ...
                                $project: {
                                    ...
                                    flags: {
                                        $filter: {
                                            input: { $ifNull: ['$flags', []] },
                                            as: 'flag',
                                            cond: { $not: { $in: ['$$flag', { $literal: [...consts.SYSTEM_FLAGS] }] } }
                                        }
                                    }
                                }
                            },
                            ...
                            counters
                                ? {
                                      $group: {
                                          _id: '$flags',
                                          total: { $sum: 1 },
                                          unseen: { $sum: { $cond: ['$unseen', 1, 0] } }
                                      }
                                  }
                                : { $group: { _id: '$flags' } },
                            { $sort: { _id: 1 } },
                            counters
                                ? { $project: { _id: 0, keyword: '$_id', total: true, unseen: true } }

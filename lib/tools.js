...
function redisConfig(defaultConfig) {
    ...
    let parsedConfig = typeof defaultConfig === 'string' ? redisUrl(defaultConfig) : defaultConfig || {};

    if (parsedConfig && parsedConfig.cluster) {
        let clusterConfig = Object.assign({}, parsedConfig);
        let nodeDefaults = {};
    ...
    return {
        ...redisDefaults,
        ...parsedConfig // unwrap other config
    };
}

function isRedisCluster(redis) {
    return !!(redis && (redis.isCluster || redis.cluster));
}

function redisHashTag(redis, value) {
    value = typeof value === 'undefined' || value === null ? '' : value.toString();
    return isRedisCluster(redis) ? `{${value}}` : value;
}
...
async function getMailboxCounter(db, mailbox, type, user) {
    // fallback (should not happen)
    if (!user) {
        let mailboxObjectId = mailbox;
        ...
        user = mailboxData && mailboxData.user;
    }

    const mailboxKey = user ? `${redisHashTag(db.redis, user)}:${mailbox}` : false;
    const cacheKey = mailboxKey ? `${type || 'total'}:${mailboxKey}` : false;

    try {
        // Check cache for pre-calculated counter value
        if (cacheKey) {
            const sum = await db.redis.get(cacheKey);
        ...
        const sum = await db.database.collection('messages').countDocuments(query);

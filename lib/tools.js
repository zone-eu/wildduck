...
async function getKeywordCounter(db, user, keyword, type) {
    try {
        ...
        return await getVersionedAccountValue(db, user, `keyword:${type || 'total'}:${keyword}`, async () => {
    ...
    for (const record of records) {
        ...
        if (counters) {
            [entry.total, entry.unseen] = await Promise.all([
                getKeywordCounter(db, user, record.path),
                getKeywordCounter(db, user, record.path, 'unseen')
            ]);
        }
        keywords.push(entry);
    }

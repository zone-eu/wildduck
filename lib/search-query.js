...
const formatTextSearchValue = (value, opts = {}) => (opts.useAndSearch === false || opts.mode === 'or' ? value : toMongoAndTextSearch(value));
...
const getMongoDBQuery = async (db, user, queryStr, opts = {}) => {
    ...
    const mergeTextEntries = (entries, queryOpts = opts) => createMongoTextQuery(entries.map(entry => getTextSearchValue(entry, queryOpts)).join(' '));
        ...
        if (node.$and && node.$and.length) {
            ...
            if (branch.$and.length > 1) {
                ...
                if (textTerms.length > 1) {
                    branch.$and = [mergeTextEntries(textTerms)].concat(nonTextTerms);
                } else {

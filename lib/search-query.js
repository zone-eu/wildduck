...
const getMongoDBQuery = async (db, user, queryStr, opts = {}) => {
    ...
    const createTextEntry = value => ({
        isTextQuery: true,
        textValue: value,
        query: createMongoTextQuery(value, opts)
    });
    ...
    let walkTree = async node => {
        ...
        if (node.$and && node.$and.length) {
            ...
            if (branch.$and.length > 1) {
                ...
                for (let entry of branch.$and) {
                    if (isTextEntry(entry)) {
                        textTerms.push(entry.textValue);
                    } else {
                ...
                if (textTerms.length > 1) {
                    branch.$and = [createMongoTextQuery(textTerms.join(' '), opts)].concat(nonTextTerms);
                } else {
            ...
            if (branch.$or.length && branch.$or.every(isTextEntry)) {
                return createMongoTextQuery(branch.$or.map(entry => entry.textValue).join(' '), { ...opts, mode: 'or' });
            }

            branch.$or = branch.$or.map(unwrapQueryEntry);
            return branch;
        } else if (node.text) {
            let branch = createTextEntry(node.text.value);

            if (node.text.negated) {
                branch = { $nor: [branch.query] };
            }

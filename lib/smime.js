...
// Startup-time probe: check whether RSA PKCS#1 v1.5 encryption is available.
// OpenSSL 3.x may have it disabled
// unless the legacy provider is loaded (e.g. --openssl-legacy-provider).
...
try {
    ...
    pkcs1v15Available = probeCt && probeCt.length > 0;
} catch (err) {
    // Not available — will be reported below
}
...
function buildEccCmsSharedInfo(wrapAlgOid, keydatalenBits) {
    ...
    return Buffer.from(new asn1js.Sequence({
        value: [
...
async function buildKeyAgreeRecipientInfo(recip, cek, cekAlg) {
    ...
    return new asn1js.Constructed({
        ...
        value: [new asn1js.Integer({value: 3}), // version
            // originator [0] EXPLICIT { [1] IMPLICIT OriginatorPublicKey }
            new asn1js.Constructed({
                idBlock: {tagClass: 3, tagNumber: 0}, value: [new asn1js.Constructed({
                    idBlock: {tagClass: 3, tagNumber: 1}, value: [ephAlgId, ephBitString]
                })]
            }), // keyEncryptionAlgorithm
            new asn1js.Sequence({
                value: [new asn1js.ObjectIdentifier({value: OID_DH_SINGLE_PASS_STD_DH_SHA256KDF}), new asn1js.Sequence({
...
function buildOaepAlgorithmIdentifier() {
    return new asn1js.Sequence({
        value: [new asn1js.ObjectIdentifier({value: OID_RSAES_OAEP}), // RSAES-OAEP-params
            new asn1js.Sequence({
                value: [// [0] hashAlgorithm = SHA-256
                    new asn1js.Constructed({
                        idBlock: {tagClass: 3, tagNumber: 0}, value: [new asn1js.Sequence({
                            value: [new asn1js.ObjectIdentifier({value: OID_SHA256})]
                        })]
                    }), // [1] maskGenAlgorithm = MGF1 with SHA-256
                    new asn1js.Constructed({
                        idBlock: {tagClass: 3, tagNumber: 1}, value: [new asn1js.Sequence({
                            value: [new asn1js.ObjectIdentifier({value: OID_MGF1}), new asn1js.Sequence({
                                value: [new asn1js.ObjectIdentifier({value: OID_SHA256})]
                            })]
                        })]
                    })]
...
function buildKeyTransRecipientInfo(recip, cek, keyTransport) {
    ...
    if (keyTransport === 'OAEP') {
        wrappedKey = crypto.publicEncrypt({
            key: recip.publicKey, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash: 'sha256'
        }, cek);
    ...
    // }
    return new asn1js.Sequence({
        value: [new asn1js.Integer({value: 0}), new asn1js.Sequence({
            value: [recip.issuer, recip.serialNumber]
        }), algId, new asn1js.OctetString({valueHex: toArrayBuffer(wrappedKey)})]
    });
}
...
class SMIMEEncryptor {
    ...
    static async encryptGCM(certs, plaintext, options) {
        ...
        try {
            ...
            let encryptedContentInfo = new asn1js.Sequence({
                value: [new asn1js.ObjectIdentifier({value: OID_DATA}), new asn1js.Sequence({
                    value: [new asn1js.ObjectIdentifier({value: OID_AES_256_GCM}), gcmParams]
                }), new asn1js.Primitive({
                    idBlock: {tagClass: 3, tagNumber: 0}, valueHex: toArrayBuffer(encrypted)
                })]
            });
            ...
            let authEnvelopedData = new asn1js.Sequence({
                value: [new asn1js.Integer({value: 0}), new asn1js.Set({value: recipientInfoValues}), encryptedContentInfo, new asn1js.OctetString({valueHex: toArrayBuffer(authTag)})]
            });

            let contentInfo = new asn1js.Sequence({
                value: [new asn1js.ObjectIdentifier({value: OID_AUTH_ENVELOPED_DATA}), new asn1js.Constructed({
                    idBlock: {tagClass: 3, tagNumber: 0}, value: [authEnvelopedData]
                })]
            });
    ...
    static async encryptCBC(certs, plaintext, options) {
        ...
        try {
            ...
            let contentInfo = new asn1js.Sequence({
                value: [new asn1js.ObjectIdentifier({value: OID_ENVELOPED_DATA}), new asn1js.Constructed({
                    idBlock: {tagClass: 3, tagNumber: 0}, value: [envelopedData]
                })]
            });
...
SMIMEEncryptor.pkcs1v15Available = pkcs1v15Available;

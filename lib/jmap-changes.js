    ...
    return {
        ...
        getChangesSince: async (user, sinceSeq) => {
            ...
            if (!sinceSeq || Number(sinceSeq) < 1) {
                ...
                // RFC 8620: Deduplicate - each ID must appear in exactly ONE array
                // Latest change type wins (created -> updated -> destroyed)
                const idMap = new Map();
                for (const e of entries) {
                    if (!idMap.has(e.id) || e.seq > idMap.get(e.id).seq) {
                        idMap.set(e.id, e);
                    }
                }
                
                const created = Array.from(idMap.values()).filter(e => e.type === 'created').map(e => e.id);
                const updated = Array.from(idMap.values()).filter(e => e.type === 'updated').map(e => e.id);
                const destroyed = Array.from(idMap.values()).filter(e => e.type === 'destroyed').map(e => e.id);
            ...
            // RFC 8620: Deduplicate - each ID must appear in exactly ONE array
            // Latest change type wins within the result set
            const idMap = new Map();
            for (const e of entries) {
                if (!idMap.has(e.id) || e.seq > idMap.get(e.id).seq) {
                    idMap.set(e.id, e);

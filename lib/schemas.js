...
const { MAX_SUB_MAILBOXES, MAX_MAILBOX_NAME_LENGTH, SYSTEM_FLAGS, MAX_KEYWORD_LENGTH, MAX_KEYWORD_LEVELS } = require('./consts');
...
const reservedKeywords = new Set([...SYSTEM_FLAGS].map(flag => flag.toLowerCase()));

const keywordSchema = Joi.string()
    .max(MAX_KEYWORD_LENGTH)
    .regex(/^[^\s\x00-\x1f\x7f"%()*[\\\]{]+$/) // eslint-disable-line no-control-regex
    .custom(
        (value, helpers) =>
            value.split('/').length <= MAX_KEYWORD_LEVELS && value.split('/').every(Boolean) ? value : helpers.error('any.invalid'),
        'keyword path validation'
    )
    .custom((value, helpers) => (value.startsWith('$') ? helpers.error('any.invalid') : value), 'internal flag validation')
    .custom((value, helpers) => (reservedKeywords.has(value.toLowerCase()) ? helpers.error('any.invalid') : value), 'custom keyword validation');

const keywordsSchema = Joi.array().items(keywordSchema).unique();

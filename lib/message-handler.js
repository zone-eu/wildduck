...
const { getKeywordMaps, keywordPathsForMessage, prepareKeywordChanges } = require('./keyword-handler');
const { publish, MARKED_HAM } = require('./events');

// index only the following headers for SEARCH
const INDEXED_HEADERS = ['to', 'cc', 'bcc', 'subject', 'from', 'sender', 'reply-to', 'message-id', 'thread-index', 'list-id', 'delivered-to'];
...
class MessageHandler {
    ...
    async _addAsync(options) {
        ...
        const assignedKeywordIds = [].concat(options.keywords || []).filter(keyword => keyword && typeof keyword.toHexString === 'function');
        const assignedKeywordMaps = assignedKeywordIds.length
            ? await getKeywordMaps(this.database, userData._id, { ids: assignedKeywordIds })
            : { records: [], byId: new Map() };
        const assignedKeywords = assignedKeywordMaps.records.filter(keyword => !keyword.deleting);
        const keywords = assignedKeywords.map(keyword => keyword._id);
        const keywordPaths = assignedKeywords.map(keyword => keyword.path);
    ...
    async _delAsync(options) {
        ...
        const keywordMaps = messageData.keywords?.length
            ? await getKeywordMaps(this.database, messageData.user, { ids: messageData.keywords })
            : { byId: new Map() };
        const keywordPaths = keywordPathsForMessage(messageData, keywordMaps.byId);
        let mailboxData;
        try {
    ...
    async _moveAsync(options) {
        ...
        if (options.updates) {
            options.updates = await prepareKeywordChanges(this.database, mailboxData.user, options.updates);
        }
        // Keyword maps are only needed for messages that have keyword IDs (or
        // when keyword changes have already supplied a prepared map). Avoid a
        // collection lookup for ordinary messages, which also keeps MOVE
        // independent of the optional keywords collection.
        let keywordMaps = options.updates?._keywordPathsById
            ? { byId: options.updates._keywordPathsById }
            : null;
        ...
        while (message !== undefined) {
            ...
            if (!keywordMaps && message.keywords && message.keywords.length) {
                keywordMaps = await getKeywordMaps(this.database, mailboxData.user);
            }

            const messageKeywordMaps = keywordMaps || { byId: new Map() };

            let unseen = message.unseen;
            let flagged = (message.flags ?? []).includes('\\Flagged');
            let keywords = keywordPathsForMessage(message, messageKeywordMaps.byId);
            ...
            Object.keys(options.updates || {}).forEach(key => {
                switch (key) {
                    ...
                    case 'keywords':
                        message.keywords = [...options.updates._keywordIds];
                        break;

                    case 'addKeywords':
                        message.keywords = [
                            ...new Map([...(message.keywords || []), ...options.updates._addKeywordIds].map(id => [id.toString(), id])).values()
                        ];
                        break;

                    case 'removeKeywords': {
                        let removeKeywords = new Set(options.updates._removeKeywordIds.map(id => id.toString()));
                        message.keywords = (message.keywords || []).filter(id => !removeKeywords.has(id.toString()));
                        break;
                    }
            ...
            const destinationKeywords = keywordPathsForMessage(message, messageKeywordMaps.byId);
    ...
    put(messageData, callback) {
        ...
        return this._put(messageData, (err, result) => {
            endMetric(err ? 'error' : 'success');
            if (!err && messageData && typeof messageData.size === 'number') {
        ...
        getMailbox((err, mailboxData) => {
            ...
            this.database.collection('mailboxes').findOneAndUpdate(
                ...
                (err, item) => {
                    ...
                    this.database.collection('messages').insertOne(messageData, { writeConcern: 'majority' }, async (err, r) => {
                        if (err) {
                            if (err.code === 11000) {
                        ...
                        try {
                            const keywordMaps = messageData.keywords?.length
                                ? await getKeywordMaps(this.database, messageData.user, { ids: messageData.keywords })
                                : { byId: new Map() };
                            keywordPaths = keywordPathsForMessage(messageData, keywordMaps.byId);
                        } catch (err) {
                            return callback(err);
                        }
    ...
    update(user, mailbox, messageQuery, changes, callback) {
        ...
        const hasKeywordChanges = changes && ['keywords', 'addKeywords', 'removeKeywords'].some(key => key in changes);
        if (!hasKeywordChanges) {
            return this._update(user, mailbox, messageQuery, changes, done);
        }

        prepareKeywordChanges(this.database, user, changes)
            .then(preparedChanges => this._update(user, mailbox, messageQuery, preparedChanges, done))
            .catch(done);
    }
    ...
    _update(user, mailbox, messageQuery, changes, callback) {
        ...
        let setKeywordIds = null;
        let addKeywordIds = [];
        let removeKeywordIds = [];
        ...
        Object.keys(changes || {}).forEach(key => {
            switch (key) {
                ...
                case 'keywords':
                    updateKeywords = true;
                    setKeywordIds = changes._keywordIds || [];
                    update = true;
                    break;

                case 'addKeywords':
                    updateKeywords = true;
                    addKeywordIds.push(...(changes._addKeywordIds || []));
                    update = true;
                    break;

                case 'removeKeywords':
                    updateKeywords = true;
                    removeKeywordIds.push(...(changes._removeKeywordIds || []));
                    update = true;
                    break;
        ...
        updateFlagsWithPipeline = updateKeywords || (addFlags.length > 0 && removeFlags.length > 0);
        ...
        const applyKeywordChanges = keywords => {
            let updatedKeywords = setKeywordIds === null ? [...(keywords || [])] : [...setKeywordIds];
            if (removeKeywordIds.length) {
                const removed = new Set(removeKeywordIds.map(id => id.toString()));
                updatedKeywords = updatedKeywords.filter(id => !removed.has(id.toString()));
            }
            if (addKeywordIds.length) {
                updatedKeywords = [
                    ...new Map([...updatedKeywords, ...addKeywordIds].map(id => [id.toString(), id])).values()
                ];
            }
        ...
        if (updateFlagsWithPipeline) {
            ...
            if (updateKeywords) {
                let keywordBase = setKeywordIds === null ? { $ifNull: ['$keywords', []] } : { $literal: setKeywordIds };
                if (removeKeywordIds.length) {
                    keywordBase = {
                        $filter: {
                            input: keywordBase,
                            as: 'keyword',
                            cond: { $not: { $in: ['$$keyword', { $literal: removeKeywordIds }] } }
                        }
                    };
                }
                updates.$set.keywords = { $setUnion: [keywordBase, { $literal: addKeywordIds }] };
            }
        } else {
            if (addFlags.length) {
                updates.$addToSet = {
        ...
        let processMailbox = (err, item) => {
            ...
            let processNext = () => {
                cursor.next((err, messageData) => {
                    ...
                    let markMessageHam = messageData => {
                        if (markHam) {
                            markHamEntries.push({
                                uid: messageData.uid,
                                message: messageData._id
                    ...
                    if (updateFlagsWithPipeline) {
                        ...
                        for (let [key, value] of Object.entries(messageUpdates.$set)) {
                            pipelineSet[key] = ['flags', 'keywords'].includes(key) ? value : { $literal: value };
                        }
                    ...
                    this.database.collection('messages').findOneAndUpdate(
                        ...
                        (err, item) => {
                            ...
                            const updatedMessageData = {
                                ...
                                keywords: applyKeywordChanges(previousMessageData.keywords)
                            };

                            const oldKeywords = new Set(keywordPathsForMessage(previousMessageData, changes._keywordPathsById || new Map()));
                            const newKeywords = new Set(keywordPathsForMessage(updatedMessageData, changes._keywordPathsById || new Map()));
        ...
        if (!update) {
            return this.database.collection('mailboxes').findOne(mailboxQuery, (err, mailboxData) => {
                processMailbox(err, mailboxData && { value: mailboxData });
            });
        }

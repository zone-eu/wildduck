...
class MessageHandler {
    ...
    async delAsync(options) {
        ...
        // Append JMAP changelog destroyed entry
        try {
            if (this.jmapChanges) {
                await this.jmapChanges.appendChange(messageData.user, { type: 'destroyed', id: messageData._id.toString() });
            }
        } catch (E) {
    ...
    async updateMessage(data, cursor, options) {
        ...
        // Append JMAP change for created message
        try {
            if (this.jmapChanges && options && options.user) {
                await this.jmapChanges.appendChange(options.user, { type: 'created', id: insertId.toString() });
            }
        } catch (E) {
            // ignore changelog errors
        }

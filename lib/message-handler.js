...
class MessageHandler {
    ...
    async _addAsync(options) {
        ...
        await ensureKeywords(this.database, userData._id, flags);
    ...
    async _moveAsync(options) {
        ...
        while (message !== undefined) {
            ...
            let keywords = tools.extractKeywords(message.flags);
            ...
            Object.keys(options.updates || {}).forEach(key => {
                switch (key) {
                    ...
                    case 'keywords':
                        message.flags = [
                            ...(message.flags ?? []).filter(flag => consts.SYSTEM_FLAGS.has(flag)),
                            ...tools.extractKeywords(options.updates.keywords)
                        ];
                        break;

                    case 'addKeywords':
                        message.flags = [...new Set([...(message.flags ?? []), ...tools.extractKeywords(options.updates.addKeywords)])];
                        break;

                    case 'removeKeywords': {
                        let removeKeywords = new Set(tools.extractKeywords(options.updates.removeKeywords));
                        message.flags = (message.flags ?? []).filter(flag => !removeKeywords.has(flag));
                        break;
                    }
    ...
    update(user, mailbox, messageQuery, changes, callback) {
        ...
        ensureKeywords(this.database, user, [...(changes.keywords || []), ...(changes.addKeywords || [])]).then(
            () => this._update(user, mailbox, messageQuery, changes, done),
            done
        );
    }
    ...
    _update(user, mailbox, messageQuery, changes, callback) {
        ...
        Object.keys(changes || {}).forEach(key => {
            switch (key) {
                ...
                case 'keywords':
                    ...
                    updateKeywords = true;
                    addFlags.push(...tools.extractKeywords(changes.keywords));
                    update = true;
                    break;

                case 'addKeywords':
                    updateKeywords = true;
                    addFlags.push(...tools.extractKeywords(changes.addKeywords));
                    update = true;
                    break;

                case 'removeKeywords':
                    updateKeywords = true;
                    removeFlags.push(...tools.extractKeywords(changes.removeKeywords));
                    update = true;
                    break;
        ...
        const applyFlagChanges = flags => {
            ...
            if (removeNonSystemFlags) {
                updatedFlags = updatedFlags.filter(flag => consts.SYSTEM_FLAGS.has(flag));
            }
        ...
        if (updateFlagsWithPipeline) {
                ...
                base = {
                    $filter: {
                        ...
                        cond: removeFlags.length
                            ? {
                                  $and: [
                                      { $in: ['$$flag', { $literal: [...consts.SYSTEM_FLAGS] }] },
                                      { $not: { $in: ['$$flag', { $literal: removeFlags }] } }
                                  ]
                              }
                            : { $in: ['$$flag', { $literal: [...consts.SYSTEM_FLAGS] }] }
                    }
            ...
            updates.$set.flags = { $setUnion: [base, { $literal: addFlags }] };
        } else {
            if (addFlags.length) {
                updates.$addToSet = {
                    flags: { $each: addFlags }
                ...
                cursor.next((err, messageData) => {
                    ...
                    if (updateFlagsWithPipeline) {
                        ...
                        for (let [key, value] of Object.entries(messageUpdates.$set)) {
                            pipelineSet[key] = key === 'flags' ? value : { $literal: value };
                        }
                    ...
                    this.database.collection('messages').findOneAndUpdate(
                        ...
                        (err, item) => {
                            ...
                            const oldKeywords = new Set(tools.extractKeywords(previousMessageData.flags));

...
class MessageHandler {
            ...
            async () =>
                new Promise(resolve => {
                    this.notifier.addEntries(
                        ...
                        () => {
                            this.loggelf({
                                short_message: '[MESSAGE-HANDLER] Notifier added entries',
                                _user: mailboxData.user,
                                _sess: options?.session && options?.session?.id,
                                _mailbox: mailboxData._id,
                                _message_id: prepared?.mimeTree?.parsedHeader?.['message-id']
    ...
    async getThreadIdAsync(userId, subject, mimeTree, referencedThreadId) {
        ...
        // Thread does not exist, but the message is referencing a thread that may have existed before, recreate thread
        ...
        let newThread;
        try {
            newThread = await this.database.collection('threads').insertOne(newThreadData);
        } catch (err) {
            if (err.code === 11000) {
                // thread already exists, return referenced thread
                return referencedThreadId;
            }
        }

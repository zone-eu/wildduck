...
class MessageHandler {
    ...
    async _moveAsync(options) {
        ...
        while (message !== undefined) {
            ...
            if (!keywordMaps) {
                keywordMaps = message.keywords && message.keywords.length
                    ? await getKeywordMaps(this.database, mailboxData.user)
                    : { byId: new Map() };
            }

            let unseen = message.unseen;
            let flagged = (message.flags ?? []).includes('\\Flagged');
            let keywords = keywordPathsForMessage(message, keywordMaps.byId);
            ...
            const destinationKeywords = keywordPathsForMessage(message, keywordMaps.byId);

...
function instrumentAsyncMessageOperation(method, operation, sourceResolver, sizeResolver) {
    const original = MessageHandler.prototype[method];
    MessageHandler.prototype[method] = async function (...args) {
        let source = sourceResolver(...args);
        let endMetric = metrics.startMessageOperation(operation, source);

        try {
            let result = await original.apply(this, args);
            endMetric('success');

            let size = typeof sizeResolver === 'function' ? sizeResolver(result, ...args) : false;
...
instrumentAsyncMessageOperation(
    'addAsync',
    'add',
    options => metrics.normalizeSource(options),
    result => result && result.data && Number(result.data.size)
);
instrumentAsyncMessageOperation('moveAsync', 'move', options => metrics.normalizeSource(options));
instrumentAsyncMessageOperation('delAsync', 'delete', options => metrics.normalizeSource(options));

const originalPut = MessageHandler.prototype.put;
MessageHandler.prototype.put = function (messageData, callback) {
    ...
    return originalPut.call(this, messageData, (err, result) => {
...
const originalUpdate = MessageHandler.prototype.update;
MessageHandler.prototype.update = function (...args) {

...
class POP3Server extends EventEmitter {
        ...
        let socketOptions = {
            ...
            SNICallback: (servername, cb) => {
                ...
                this.options.SNICallback(opts, (err, context) => {
                    if (err) {
                        this.loggelf({
                            short_message: '[POP3SNIERR] ' + (err.message || 'SNI error'),
    ...
    _handleProxy(socket, callback) {
        ...
        let done = (err, opts) => {
            ...
            if (err) {
                ...
                // (e.g. EPIPE while flushing "-ERR") must not surface as an
                // uncaughtException.
                socket.on('error', () => {
                    // ignore
                });
    ...
    connect(socket, socketOptions) {
        ...
        metrics.connectionStarted('pop3', connection.secured || connection.secure);

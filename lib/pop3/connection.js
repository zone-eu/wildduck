...
class POP3Connection extends EventEmitter {
    ...
    send(payload) {
        ...
        this.logger.debug(
            ...
            this.id + ' S:',
            (payload.length < 128 ? payload : payload.slice(0, 128) + '... +' + (payload.length - 128) + ' B').replace(/\r?\n/g, '\\n')
        );
        this.write(payload + '\r\n');
    }
    ...
    _onError(err) {
        ...
        if (typeof this._server.loggelf === 'function') {
            this._server.loggelf({
                short_message: '[POP3ERROR] ' + (err && err.message ? err.message : 'Connection error'),
    ...
    processQueue() {
        ...
        let recordMetric = result => {
            ...
            metrics.recordPop3Command(command || 'unknown', result, diff[0] + diff[1] / 1e9);
        };
    ...
    command_RETR(args, next) {
        ...
        this._server.onFetchMessage(message, this.session, (err, stream) => {
            ...
            dataStream.once('error', err => next(err));

            // Listen for 'end' on dataStream (not source stream) to ensure the
            // terminating dot has been written before processing next pipelined command
            dataStream.once('end', () => {
                ...
                // update counters if ttlcounter is available
                if (typeof stream?.options?.ttlcounter === 'function') {
                    stream.options.ttlcounter(stream.options.key, stream.bytes, stream.options.maxBytes, false, finishRetr);
                } else {
                    finishRetr();
                }
            });
    ...
    command_TOP(args, next) {
        ...
        this._server.onFetchMessage(message, this.session, (err, stream) => {
                    ...
                    if (!finished) {
                        ...
                        if (!headers) {
                            ...
                            if ((match = data.match(/\r?\n\r?\n/))) {
                                headers = data.slice(0, match.index + match[0].length);
                                if (data.length > headers.length) {
                                    data = data.slice(headers.length);
                                } else {
                                    data = '';
                                }
                        ...
                        if (headers) {
                            ...
                            while (!finished && (match = data.match(/\r?\n/))) {
                                let line = data.slice(0, match.index + match[0].length);
                                linesSent++;
                                if (data.length > line.length) {
                                    data = data.slice(line.length);
                                } else {
                                    data = '';
                                }
            ...
            stream.once('end', () => {
                ...
                // update counters if ttlcounter is available
                if (typeof stream?.options?.ttlcounter === 'function') {
                    stream.options.ttlcounter(stream.options.key, totalSentBytes, stream.options.maxBytes, false, finishTop);
                } else {
                    finishTop();
                }
            });
    ...
    _resetSocketTimeout() {
        ...
        this._socket.setTimeout(this._server.options.socketTimeout || SOCKET_TIMEOUT);
    }
}

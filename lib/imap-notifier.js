...
class ImapNotifier extends EventEmitter {
    constructor(options) {
        ...
        const redisConfig = options.redis ? false : tools.redisConfig(config.dbs.redis);
        this.redis = options.redis || (redisConfig.cluster ? new Redis.Cluster(redisConfig.nodes, redisConfig.options) : new Redis(redisConfig));
        ...
        this.subscribeFnName = tools.isRedisCluster(this.redis) ? 'ssubscribe' : 'subscribe';
        this.publishFnName = tools.isRedisCluster(this.redis) ? 'spublish' : 'publish';
        this.messageEventName = tools.isRedisCluster(this.redis) ? 'smessage' : 'message';
        ...
        this.subscriber = this.redis.duplicate([], tools.isRedisCluster(this.redis) ? { shardedSubscribers: true } : {}); // connection agnostic (standalone or Cluster)
        ...
        this.subscriber.on(this.messageEventName, (channel, message) => {
        ...
        this.subscriber[this.subscribeFnName](this._userRegistryState.workerChannel);
        this.subscriber[this.subscribeFnName](LEGACY_CHANNEL);
            ...
            this._usesWorkerWdEvents()
                .then(useWorkerWdEvents => {
                    if (!useWorkerWdEvents) {
                        return this.redis[this.publishFnName](LEGACY_CHANNEL, data);
                    }
                    ...
                    return this.redis.zrangebyscore(key, minScore, '+inf').then(workers => {
                        ...
                        if (!tools.isRedisCluster(this.redis)) {
                            const pipeline = this.redis.pipeline();
                            workers.forEach(workerId => pipeline[this.publishFnName](this._getWorkerChannel(workerId), data));
                            return pipeline.exec();
                        }

                        return Promise.all(workers.map(workerId => this.redis[this.publishFnName](this._getWorkerChannel(workerId), data)));
                    });
                })
                .catch(() => false);
        });
    ...
    _refreshUserWorkers() {
        ...
        for (let userId of counts.keys()) {
            ...
            if (!tools.isRedisCluster(redis)) {
                commands.push(
                    ['zremrangebyscore', key, 0, minScore],
                    ['zadd', key, now, this._userRegistryState.workerId],
                    ['expire', key, USER_WORKERS_TTL + 60]
                );
                continue;
            }
        ...
        Promise.all(commands).catch(() => false);
    }
    ...
    async updateCounters(entries) {
        ...
        (Array.isArray(entries) ? entries : [].concat(entries || [])).forEach(entry => {
            ...
            if (!counters.has(m)) {
                counters.set(m, { user: entry.user && entry.user.toString(), total: 0, unseen: 0, unseenChange: false });
            }
        ...
        for (let row of Array.from(counters)) {
            ...
            let mailboxKey = `${tools.redisHashTag(this.redis, delta.user)}:${mailbox}`;

            await this.redis.cachedcounter(`total:${mailboxKey}`, delta.total, consts.MAILBOX_COUNTER_TTL);
            ...
            if (delta.unseenChange) {
                ...
                await this.redis.del(`unseen:${mailboxKey}`);
            } else if (delta.unseen) {
                await this.redis.cachedcounter(`unseen:${mailboxKey}`, delta.unseen, consts.MAILBOX_COUNTER_TTL);
            }
        }
    }
    ...
    allocateConnection(data, callback) {
        ...
        let rlkey = `lim:${data.service}:${tools.redisHashTag(this.redis, data.user.toString())}`;
    ...
    releaseConnection(data, callback) {
        ...
        let rlkey = `lim:${entry.service}:${tools.redisHashTag(this.redis, entry.user.toString())}`;

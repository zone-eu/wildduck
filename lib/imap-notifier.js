...
const { ensureKeywords } = require('./keyword-handler');

const LEGACY_CHANNEL = 'wd_events';
...
const USER_WORKERS_TTL_MS = USER_WORKERS_TTL * 1000;
const USE_WORKER_WD_EVENTS_SETTING = 'const:imap:use_wd_worker_channels';
const USE_WORKER_WD_EVENTS_CACHE_TTL = 5000; // ms
const WORKER_ID = `${config?.dbs?.workerId || os.hostname()}:${process.pid}`;
const PROCESS_USER_REGISTRY_STATE = {
    counts: new Map(),
    listeners: 0
};
...
class ImapNotifier extends EventEmitter {
    constructor(options) {
        ...
        this._userRegistryState = {
            counts: new Map(),
            workerId: WORKER_ID,
            workerChannel: `${WORKER_CHANNEL_PREFIX}${WORKER_ID}`,
            useWorkerWdEvents: false,
            useWorkerWdEventsUpdated: 0,
            useWorkerWdEventsPending: null,
            timer: null,
            redis: null
        };
        ...
        this.subscriber.on('message', (channel, message) => {
            if (channel !== this._userRegistryState.workerChannel && channel !== LEGACY_CHANNEL) {
                return;
            }

            let data;
            let needFullParse = !(message.length === 32 && message[2] === 'e' && message[5] === '"' && message[6 + 24] === '"');
        ...
        this.subscriber.subscribe(this._userRegistryState.workerChannel);
        this.subscriber.subscribe(LEGACY_CHANNEL);
    ...
    removeListener(session, handler) {
        let userId = session.user.id.toString();
        let listenerCount = this._listeners.listenerCount(userId);
        this._listeners.removeListener(userId, handler);
        if (this._listeners.listenerCount(userId) < listenerCount) {
    ...
    addEntries(mailbox, entries, callback) {
        ...
        let pushToJournal = async () => {
            // Persist discovered paths independently of Redis counter availability.
            const keywordUsers = new Map();
            for (const entry of entries) {
                if (entry.user && (entry.command === 'EXISTS' || entry.command === 'FETCH')) {
                    const userKey = entry.user.toString();
                    if (!keywordUsers.has(userKey)) {
                        keywordUsers.set(userKey, new Set());
                    }
                    for (const keyword of [...(entry.keywords || []), ...(entry.flags || []), ...(entry.addedKeywords || [])]) {
                        keywordUsers.get(userKey).add(keyword);
                    }
                }
            }
            for (const [user, keywords] of keywordUsers) {
                await ensureKeywords(this.database, user, [...keywords]);
            }
            try {
                await this.prepareAccountCounterInvalidations(entries);
            } catch (err) {
                this.logger.error('Failed to prepare account counter invalidations', err.message);
            }

            for (let entry of entries) {
                if ((entry.command === 'EXISTS' && entry.junk) || entry.markHam) {
                    await publish(this.redis, {
                        ev: entry.command === 'EXISTS' && entry.junk > 0 ? MARKED_SPAM : MARKED_HAM,
            ...
            metrics.recordJournalEntries(entries);
            ...
            try {
                await this.updateAccountCounters(entries);
            } catch (err) {
                this.logger.error('Failed to invalidate account counters', err.message);
            }
    ...
    fire(user, payload) {
        ...
        setImmediate(() => {
            ...
            this._usesWorkerWdEvents()
                .then(useWorkerWdEvents => {
                    if (!useWorkerWdEvents) {
                        metrics.recordNotification('legacy');
                        return this.redis.publish(LEGACY_CHANNEL, data);
                    }

                    const key = this._getUserWorkersKey(userId);
                    const minScore = Date.now() - USER_WORKERS_TTL_MS;
                    // send message only to workers that actually service this user
                    return this.redis.zrangebyscore(key, minScore, '+inf').then(workers => {
                        if (!workers || !workers.length) {
                            metrics.recordNotification('no-workers');
                            return;
                        }

                        metrics.recordNotification('worker');
    ...
    _usesWorkerWdEvents() {
        // Use promise memoization
        if (this._userRegistryState.useWorkerWdEventsUpdated > Date.now() - USE_WORKER_WD_EVENTS_CACHE_TTL) {
            return Promise.resolve(this._userRegistryState.useWorkerWdEvents);
        }

        // return memoized promise
        if (this._userRegistryState.useWorkerWdEventsPending) {
            return this._userRegistryState.useWorkerWdEventsPending;
        }

        // Return settingsHandler async request promise
        this._userRegistryState.useWorkerWdEventsPending = this.settingsHandler
            .get(USE_WORKER_WD_EVENTS_SETTING, { default: false })
            .then(useWorkerWdEvents => {
                this._userRegistryState.useWorkerWdEvents = !!useWorkerWdEvents;
                this._userRegistryState.useWorkerWdEventsUpdated = Date.now();
                return this._userRegistryState.useWorkerWdEvents;
            })
            .catch(() => {
                this._userRegistryState.useWorkerWdEventsUpdated = Date.now();
                return this._userRegistryState.useWorkerWdEvents;
            ...
            .finally(() => {
                this._userRegistryState.useWorkerWdEventsPending = null;
            });
    ...
    _incrementUserListener(userId) {
        ...
        let processCount = (PROCESS_USER_REGISTRY_STATE.counts.get(userId) || 0) + 1;
        PROCESS_USER_REGISTRY_STATE.counts.set(userId, processCount);
        PROCESS_USER_REGISTRY_STATE.listeners++;

        if (processCount === 1) {
            this._registerUserWorker(userId);
        }
        this._updateListenerMetrics();
    }
    ...
    _decrementUserListener(userId) {
        ...
        let processCount = PROCESS_USER_REGISTRY_STATE.counts.get(userId) || 0;
        if (processCount <= 1) {
            PROCESS_USER_REGISTRY_STATE.counts.delete(userId);
            this._unregisterUserWorker(userId);
        } else {
            PROCESS_USER_REGISTRY_STATE.counts.set(userId, processCount - 1);
        }
        PROCESS_USER_REGISTRY_STATE.listeners = Math.max(PROCESS_USER_REGISTRY_STATE.listeners - 1, 0);
        this._updateListenerMetrics();
    }

    _updateListenerMetrics() {
        metrics.setNotificationListenerCounts(PROCESS_USER_REGISTRY_STATE.counts.size, PROCESS_USER_REGISTRY_STATE.listeners);
    }
    ...
    _registerUserWorker(userId) {
        ...
        this.redis
            .pipeline()
            .zadd(key, Date.now(), this._userRegistryState.workerId)
    ...
    _unregisterUserWorker(userId) {
        let key = this._getUserWorkersKey(userId);
        this.redis.zrem(key, this._userRegistryState.workerId).catch(() => false);
    }
    ...
    _refreshUserWorkers() {
        ...
        for (let userId of counts.keys()) {
            ...
            pipeline.zadd(key, now, this._userRegistryState.workerId);
    ...
    async prepareAccountCounterInvalidations(entries) {
        const deletions = (Array.isArray(entries) ? entries : [entries]).filter(entry => entry?.command === 'DELETE' && entry.user);
        const entriesByUser = new Map();

        for (const entry of deletions) {
            entry.flaggedCounterInvalidated = true;
            const userKey = entry.user.toString();
            if (!entriesByUser.has(userKey)) {
                entriesByUser.set(userKey, []);
            }
            entriesByUser.get(userKey).push(entry);
        }

        for (const [userKey, userEntries] of entriesByUser) {
            const counterKeywords = await tools.getTrackedKeywords(this.redis, userKey);
            for (const entry of userEntries) {
                entry.counterKeywords = counterKeywords;
            }
        }
    }

    async updateAccountCounters(entries) {
        const list = Array.isArray(entries) ? entries : [].concat(entries || []);
        ...
        const keywords = list.flatMap(entry =>
            tools.extractKeywords([...(entry?.keywords ?? []), ...(entry?.flags ?? []), ...(entry?.addedKeywords ?? []), ...(entry?.removedKeywords ?? [])])
        );
        await tools.trackKeywords(this.redis, user, keywords);

        const countersChanged = list.some(entry => {
            if (entry?.command === 'DELETE') {
                return true;
            }
            const entryKeywords = tools.extractKeywords([
                ...(entry?.keywords ?? []),
                ...(entry?.addedKeywords ?? []),
                ...(entry?.removedKeywords ?? []),
                ...(entry?.unseenChange ? entry?.flags ?? [] : [])
            ]);
            return (
                entryKeywords.length > 0 ||
                entry?.flagged === true ||
                typeof entry?.flaggedChangedTo === 'boolean' ||
                (entry?.unseenChange && (entry?.flags ?? []).includes('\\Flagged'))
            );
        });
        if (countersChanged) {
            await tools.bumpAccountCounterVersion(this.redis, user);
        }
    }

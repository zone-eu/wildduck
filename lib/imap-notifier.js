...
const USER_WORKERS_TTL = Math.max(Number(config?.imap?.notifyUserWorkersTtl) || 90, 30);
const USER_WORKERS_REFRESH = Math.max(Number(config?.imap?.notifyUserWorkersRefresh) || Math.floor(USER_WORKERS_TTL / 2), 10);
const USER_WORKERS_TTL_MS = USER_WORKERS_TTL * 1000;
const USER_REGISTRY_STATE = {
    counts: new Map(),
    workerId: (process.env.WD_WORKER_ID || process.pid).toString(),
    workerChannel: null,
    timer: null,
    redis: null
};
USER_REGISTRY_STATE.workerChannel = `${WORKER_CHANNEL_PREFIX}${USER_REGISTRY_STATE.workerId}`;
    ...
    fire(user, payload) {
        ...
        setImmediate(() => {
            ...
            this.redis
                ...
                .zrangebyscore(key, minScore, '+inf')
                .exec()
                .then(res => {

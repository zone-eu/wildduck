...
class ImapNotifier extends EventEmitter {
    constructor(options) {
        ...
        this.subscribeFnName = this.redis.isCluster ? 'ssubscribe' : 'subscribe';
        this.publishFnName = this.redis.isCluster ? 'spublish' : 'publish';
        this.messageEventName = this.redis.isCluster ? 'smessage' : 'message';
        ...
        this.subscriber = this.redis.duplicate([], this.redis.isCluster ? { shardedSubscribers: true } : {}); // connection agnostic (standalone or Cluster)
    ...
    async updateCounters(entries) {
        ...
        for (let row of Array.from(counters)) {
            ...
            let mailboxKey = `{${delta.user}}:${mailbox}`;
    ...
    allocateConnection(data, callback) {
        ...
        let rlkey = `lim:${data.service}:{${data.user.toString()}}`;
    ...
    releaseConnection(data, callback) {
        ...
        let rlkey = `lim:${entry.service}:{${entry.user.toString()}}`;

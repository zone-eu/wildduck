...
class ImapNotifier extends EventEmitter {
        ...
        for (let entry of entries) {
            ...
            switch (entry.command) {
                ...
                case 'FETCH': {
                    ...
                    const previousMessageUnseen = entry.unseenChange ? !messageUnseen : messageUnseen;
                    const addedKeywords = new Set(tools.extractKeywords(entry.addedKeywords));
                    ...
                    for (let keyword of tools.extractKeywords(entry.removedKeywords)) {
                        this.addKeywordDelta(deltas, keyword, -1, previousMessageUnseen ? -1 : 0);
                    }
    ...
    async getCachedKeywordCounters(userKey) {
        const keywords = new Set();

        for (const type of ['total', 'unseen']) {
            const prefix = `kw:${type}:${userKey}:`;
            let cursor = '0';
            ...
            do {
                ...
                for (const key of result[1]) {
                    keywords.add(key.slice(prefix.length));
                }
            } while (cursor !== '0');
        }
    ...
    async prepareAccountCounterInvalidations(entries) {
        ...
        for (const [userKey, userEntries] of entriesByUser) {
            const counterKeywords = await this.getCachedKeywordCounters(userKey);
    ...
    async updateKeywordCounters(entries) {
        ...
        const invalidatedKeywords = new Set(list.flatMap(entry => tools.extractKeywords(entry?.counterKeywords)));
        if (invalidatedKeywords.size) {
            await this.redis.del(...[...invalidatedKeywords].flatMap(keyword => [`kw:total:${userKey}:${keyword}`, `kw:unseen:${userKey}:${keyword}`]));
        }
    }
            ...
            switch (entry.command) {
                ...
                case 'FETCH': {
                    ...
                    if (entry.flaggedChangedTo === true) {
                        ...
                        delta.total -= 1;
                        if (previousMessageUnseen) {
                            delta.unseen -= 1;
                        }
                    }

                    if (entry.unseenChange && messageFlagged && typeof entry.flaggedChangedTo !== 'boolean') {
    ...
    async updateFlaggedCounters(entries) {
        ...
        if (delta.total || delta.unseen) {
            await this.applyFlaggedDelta(userKey, delta);
        }

        if (list.some(entry => entry?.flaggedCounterInvalidated)) {
            await this.redis.del(`fl:total:${userKey}`, `fl:unseen:${userKey}`);
        }
    }

...
class ImapNotifier extends EventEmitter {
    ...
    addEntries(mailbox, entries, callback) {
        ...
        let pushToJournal = async mailboxData => {
            ...
            // trigger APNs push for new messages (fire-and-forget, debounced)
            if (this.apn && mailboxData && mailboxData._id) {
                let hasNewMessages = entries.some(entry => entry.command === 'EXISTS');
                if (hasNewMessages) {
                    this.apn.notify(mailboxData.user, mailboxData._id);
                }
            }

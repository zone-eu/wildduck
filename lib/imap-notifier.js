...
const PUBLISH_LEGACY_WD_EVENTS = !!config?.imap?.publishLegacyWdEvents;
...
class ImapNotifier extends EventEmitter {
    constructor(options) {
        ...
        this.subscriber.on('message', (channel, message) => {
            ...
            if (message.length === 32 && message[2] === 'e' && message[5] === '"' && message[6 + 24] === '"') {
                ...
                // there is only e and c, no p -> no need for full parse
                needFullParse = false;
                compat = true;
            }
            ...
            // In compatibility mode publish to both the worker-specific channel and
            // the legacy shared channel. Upgraded nodes ignore the legacy copy.
            if (channel === LEGACY_CHANNEL && data.c) {
                return;
            }
    ...
    fire(user, payload) {
        ...
        setImmediate(() => {
            ...
            let legacyData = PUBLISH_LEGACY_WD_EVENTS
                ? JSON.stringify({
                      e: user.toString(),
                      p: payload,
                      c: true
            ...
            this.redis
                ...
                .then(workers => {
                    if ((!workers || !workers.length) && !legacyData) {
                        return;
                    }
                    ...
                    if (legacyData) {
                        pipeline.publish(LEGACY_CHANNEL, legacyData);
                    }
